...
<!-- Run 'bd onboard --output .beans/BD_GUIDE.md' to regenerate -->
...
6. **Commit together**: Always commit the `.beans/issues.jsonl` file together with the code changes so issue state stays in sync with code state
...
```bash
bd onboard --output .beans/BD_GUIDE.md
```
