...
- **Shell completions without database** - Work without .beans/ (#1118)
...
- **Symlinked .beans directories** - Correct routing for symlinks (#1112)
...
- **Redirect + sync-branch incompatibility** - bd sync works correctly in redirected repos (bd-wayc3)
  ...
  - Proper handling of .beans/redirect during sync operations
...
- **Sync robustness**
  ...
  - Force-add .beans in worktree for contributor mode (#947)
...
- **Sync with redirect** - Use inline import for `--import-only` with .beans/redirect (bd-ysal)
  - Handle .beans/redirect in git status checks (bd-arjb)
...
- **`.beans/last-touched`** - Restored to gitignore template (GH#838)
...
- **`.beans/redirect` not committed** - Prevents worktree conflicts (GH#814)
...
- **MCP redirect support** - Plugin follows .beans/redirect files
...
- **`bd doctor --fix` redirect handling**
  - Properly follows .beans/redirect files
...
- **Prefix-based routing** - Cross-rig command routing
  ...
  - Routes stored in `~/gt/.beans/routes.jsonl`
...
- **Simplified wisp architecture** - Single database model
  ...
  - Removed separate `.beans-wisp/` directory
...
- **bd sync commits non-.beans files** - Sync isolation
  - Sync operations now only commit .beans/ directory changes
...
- **Formula system** (bd-weu8, bd-wa2l) - Declarative workflow templates
  ...
  - Search paths: `.beans/formulas/`, `~/.beans/formulas/`, `~/gt/.beans/formulas/`
...
- **Config-based close hooks** - Custom scripts on issue close
  - Configure hooks in `.beans/config.yaml`
...
- **Cross-project dependencies** (bd-66w1, bd-om4a) - Reference issues across repos
  ...
  - Configure additional repos in `.beans/config.yaml`
...
- **Multi-repo config uses YAML** (GH#683) - `bd repo add/remove` now writes to `.beans/config.yaml`
...
- **Agent audit trail** (GH#649) - Append-only logging for AI agent interactions
  - New `.beans/interactions.jsonl` audit file
...
- **Windows winget manifest** (GH#524) - Prepare beads for Windows Package Manager
  ...
  - Once merged to microsoft/winget-pkgs: `winget install SteveYegge.beans`
...
- **`bd mail` commands** - Inter-agent messaging
  ...
  - Identity via `BEADS_IDENTITY` env var or `.beans/config.json`
...
- **Hooks system** - Extensible event notifications
  - `.beans/hooks/on_create` - Runs after issue creation
  - `.beans/hooks/on_update` - Runs after issue update
  - `.beans/hooks/on_close` - Runs after issue close
  - `.beans/hooks/on_message` - Runs after message send
...
- **`bd reset` command** (GH#505) - Complete beads removal from a repository
  - Removes `.beans/` directory and all associated data
...
- **Enhanced Git Worktree Support**
  - Shared `.beans` database across all worktrees
...
- **Pre-commit hook with removed `.beans`** (GH#483) - No longer blocks commits after beads removal
...
- **Nested worktree detection** (GH#509) - Correctly finds `.beans/` in parent repo
...
- **PR check for `.beans/issues.jsonl` changes** - Rejects accidental database commits
...
- **`bd migrate-sync` command** - Migrate to sync.branch workflow
  ...
  - Keeps main branch clean of .beans/ commits
...
- **sync.branch Workflow Fixes**
  - Fixed .beans/ restoration from branch after sync
...
- **Snapshot Cleanup**: Properly cleans up snapshot files after sync
  - Removes `.beans/*.snapshot` files that could cause conflicts
...
- **bd sync Commit Scope**
  - `bd sync` now only commits `.beans/` files, not other staged files
...
- **bd sync commit scope**: Now commits entire `.beans/` directory before pull
...
- **Deletion Propagation**: Deletions now sync across clones via deletions manifest
  - New `.beans/deletions.jsonl` tracks deleted issues with timestamp, actor, reason
...
- **Stealth Mode**: New `bd init --stealth` flag for invisible beads usage (#381)
  - Adds `.beans/` to project's `.gitignore` automatically
...
- **bd clean**: Remove temporary merge artifacts (e8355c2)
  ...
  - Helps maintain clean `.beans/` directory after merges
...
- **.beans/README.md Generation**: Auto-generated during `bd init` (bd-m7ge, e1c8853)
...
- **Auto-upgrade .beans/.gitignore** (#300, f4a2f87)
...
- **Documentation Improvements**:
  ...
  - How to resolve merge conflicts in .beans/beads.jsonl (4985a68)
...
- **Pull Requests**:
  ...
  - #300: Automatic .beans/.gitignore upgrade
...
- **bd-auf1: Auto-cleanup snapshot files after successful merge**
  - `.beans/` no longer accumulates orphaned `.base`, `.ours`, `.theirs` snapshot files after merges
...
- **Nested .beans Directories**: Prevent creating nested .beans directories
...
- **Git Merge Artifacts** (41b1a21): Ignore merge artifacts in `.beans/.gitignore`
...
- **Intelligent Merge Driver** (bd-omx1, 52c5059): Auto-configured git merge driver for JSONL conflict resolution
  ...
  - Eliminates most git merge conflicts in `.beans/beads.jsonl`
...
- **DevContainer Support** (247e659): Automatic bd setup in GitHub Codespaces
  ...
  - Auto-detects existing `.beans/` and imports on startup
...
- **Protected Branch Workflow**:
  ...
  - Git worktree management with sparse checkout for sync branches
    - Only checks out `.beans/` in worktrees, minimal disk usage
...
- Git worktree management for sync branches uses sparse checkout (`.beans/` only)
...
- **Hash-Based IDs**: New collision-resistant ID system (bd-168, bd-166, bd-167)
  ...
  - Opt-in via `.beans/config.toml` with `id_mode = "hash"`
...
- **Critical**: Database reinitialization data loss bug (bd-130, DATABASE_REINIT_BUG.md)
  - Fixed bug where removing `.beans/` and running `bd init` would lose git-tracked issues
...
- **BREAKING**: Removed global daemon socket fallback
  - Each project now must use its own local daemon (.beans/bd.sock)
  ...
  - Warning displayed if old global socket (~/.beans/bd.sock) is found
...
- **Critical**: Concurrent temp file collisions (bd-306, bd-373)
  ...
  - Now uses PID suffix for temp files: `.beans/issues.jsonl.tmp.12345`
...
- **Database Discovery**: Auto-find database in project hierarchy
  ...
  - Falls back to `~/.beans/default.db`
...
1. Run `bd export -o .beans/issues.jsonl` to create the text file
2. Commit `.beans/issues.jsonl` to git
3. Add `.beans/*.db` to `.gitignore`

New collaborators can clone the repo and run:
```bash
bd import -i .beans/issues.jsonl
```
