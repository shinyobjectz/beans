...
cat > "$WINGET_DIR/SteveYegge.beans.yaml" << EOF
# yaml-language-server: \$schema=https://aka.ms/winget-manifest.version.1.6.0.schema.json
PackageIdentifier: SteveYegge.beans
...
cat > "$WINGET_DIR/SteveYegge.beans.installer.yaml" << EOF
...
cat > "$WINGET_DIR/SteveYegge.beans.locale.en-US.yaml" << EOF
