...
3. Without correct CWD, `bd` discovers wrong database or falls back to `~/.beans`
...
2. Walks up directory tree to find `.beans/*.db`
...
```json
{
  "mcpServers": {
    "beads-repo1": {
      ...
      "args": ["beads-mcp"],
      "env": {
        "BEADS_DB": "/path/to/repo1/.beans/prefix.db"
      }
    },
    "beads-repo2": {
      "command": "uvx",
      "args": ["beads-mcp"],
      "env": {
        "BEADS_DB": "/path/to/repo2/.beans/prefix.db"
      }
    }
  }
}
