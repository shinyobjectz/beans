...
1. MCP server checks for local daemon socket (`.beans/bd.sock`) in your current workspace
...
- Git submodules with `.beans` directories use local context
...
**Submodule handling**: Submodules with their own `.beans` directory are treated as separate projects.
