...
The daemon will:
- Listen on `.beans/bd.sock` (Windows: file stores loopback TCP metadata)
...
Claude/Amp → Single MCP Server → Daemon Client → Daemon → Correct Database
          ...
          Uses set_context to pass workspace_root
                  ↓
          Daemon uses cwd to find .beans/*.db
```
...
- Each repo has `.beans/*.db` properly initialized
...
```json
{
  "mcpServers": {
    "beads-adar": {
      "command": "beads-mcp",
      "env": {
        "BEADS_DB": "/path/to/adar/.beans/bd.db"
      }
    },
    "beads-wyvern": {
      "command": "beads-mcp",
      "env": {
        "BEADS_DB": "/path/to/wyvern/.beans/wy.db"
      }
    }
  }
}
