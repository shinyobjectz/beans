---
description: How to resolve merge conflicts in .beans/issues.jsonl
---

# Resolving `issues.jsonl` Merge Conflicts

If you encounter a merge conflict in `.beans/issues.jsonl` that doesn't have standard git conflict markers (or if `bd merge` failed automatically), follow this procedure.
...
```powershell
git show :1:.beans/issues.jsonl > beads.base.jsonl
git show :2:.beans/issues.jsonl > beads.ours.jsonl
git show :3:.beans/issues.jsonl > beads.theirs.jsonl
```
...
```powershell
cp beads.merged.jsonl .beans/issues.jsonl
```
