...
Notes:
- SQLite backend stores data in `.beans/beads.db`.
- Dolt backend stores data in `.beans/dolt/` and records `"database": "dolt"` in `.beans/metadata.json`.
...
By default, the database is in `.beans/beads.db` (gitignored).

The JSONL file `.beans/issues.jsonl` is git-tracked and syncs automatically.
