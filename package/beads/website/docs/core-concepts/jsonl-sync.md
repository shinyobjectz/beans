...
```
SQLite DB (.beans/beads.db, gitignored)
    ↕ auto-sync (5s debounce)
JSONL (.beans/issues.jsonl, git-tracked)
    ↕ git push/pull
...
git checkout --ours .beans/issues.jsonl   # or --theirs
bd import -i .beans/issues.jsonl
bd sync
```
...
Deleted issues are tracked in `.beans/deletions.jsonl`:
...
bd import -i .beans/issues.jsonl --orphan-handling allow
```
