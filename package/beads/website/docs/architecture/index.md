...
flowchart TD
    subgraph GIT["🗂️ Layer 1: Git Repository"]
        G[(".beans/*.jsonl<br/><i>Historical Source of Truth</i>")]
    end
...
**Location:** `.beans/beads.db`

**Why SQLite?**
- Instant queries (no network)
- Complex filtering and sorting
- Derived from JSONL (always rebuildable)
- Safe to delete and rebuild: `rm .beans/beads.db* && bd sync --import-only`
...
2. **Lost JSONL?** → Recover from Git history: `git checkout HEAD~1 -- .beans/issues.jsonl`
...
rm .beans/beads.db*          # Remove potentially corrupted database
