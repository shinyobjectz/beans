...
Beads uses git for:
- **JSONL sync** - Issues stored in `.beans/issues.jsonl`
- **Deletion tracking** - `.beans/deletions.jsonl`
...
.beans/deletions.jsonl merge=beads
```
...
Why: Daemon uses `.beans/bd.sock` which conflicts across worktrees.
