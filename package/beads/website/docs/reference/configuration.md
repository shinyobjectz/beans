...
1. **Project config**: `.beans/config.toml` (highest priority)
2. **User config**: `~/.beans/config.toml`
...
```toml
[database]
path = ".beans/beads.db"     # Database file location
```
...
```toml
[export]
path = ".beans/issues.jsonl"  # Export file location
