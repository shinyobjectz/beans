...
This installs hooks to `.beans-hooks/` (a versioned directory) instead of `.git/hooks/`, and configures git to use them via `git config core.hooksPath .beans-hooks`.
...
After running `bd hooks install --shared`, commit `.beans-hooks/` to your repository:
...
This:
1. Exports any pending database changes to `.beans/issues.jsonl`
...
git status --porcelain .beans/*.jsonl  # Check for uncommitted changes
```
...
- Only runs if `.beans/issues.jsonl` exists (also checks `issues.jsonl` for backward compat)
...
- See [bd-51](../../.beans/bd-51) for the race condition bug report
