...
2. ✅ Prompt you to create a planning repo (`~/.beans-planning` by default)
...
# This issue goes to ~/.beans-planning automatically!
```
...
Beads automatically routes this to your planning repo (`~/.beans-planning/.beans/issues.jsonl`), not the current repo.
...
bd list --source-repo ~/.beans-planning
```
...
Your planning issues in `~/.beans-planning` **never appear in PRs**.
...
The wizard configures these settings in `.beans/beads.db`:
...
bd list --source-repo ~/.beans-planning    # Planning repo only
```
