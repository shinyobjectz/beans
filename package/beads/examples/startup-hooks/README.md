...
- ✅ Persists version tracking in `.beans/metadata.json`
...
- **.beans directory**: Must exist in current project

### How It Works

1. **Version Detection**: Reads current bd version and compares to `.beans/metadata.json`
...
A: Check that `.beans/metadata.json` exists and contains `last_bd_version` field
