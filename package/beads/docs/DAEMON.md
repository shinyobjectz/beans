...
- Socket at `.beans/bd.sock` (`.beans/bd.pipe` on Windows)
...
#     "pid": 12345,
#     "socket": "/Users/alice/projects/webapp/.beans/bd.sock",
#     "version": "0.21.0",
#     "uptime_seconds": 3600
#   }
# ]
...
- Socket file stale: `rm .beans/bd.sock` (auto-cleans on next start)
...
│  │      ├─ .beans/issues.jsonl (file changes)                │   │
...
2. `remote-sync-interval` in `.beans/config.yaml`
...
When `.beans/.exclusive-lock` file exists:
...
```bash
# Create lock
echo '{"holder":"my-tool","pid":'$$',"hostname":"'$(hostname)'","started_at":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","version":"1.0.0"}' > .beans/.exclusive-lock

# Do work (daemon won't interfere)
bd create "My issue" -p 1

# Release lock
rm .beans/.exclusive-lock
```
