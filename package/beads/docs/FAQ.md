...
git clone <repo>
cd <repo>
bd init  # Auto-imports from .beans/issues.jsonl

# Or initialize new project:
cd ~/my-project
bd init  # Creates .beans/, sets up daemon
git add .beans/
...
**How auto-import works:** The first bd command after `git pull` detects that `.beans/issues.jsonl` is newer than the database and automatically imports it. There's no background daemon watching for changes - the check happens when you run a bd command.
...
Each project gets its own `.beans/` directory with its own database and JSONL file. bd auto-discovers the correct database based on your current directory (walks up like git).

**Multi-project scenarios work seamlessly:**
- Multiple agents working on different projects simultaneously → No conflicts
- Same machine, different repos → Each finds its own `.beans/*.db` automatically
- Agents in subdirectories → bd walks up to find the project root (like git)
- **Per-project daemons** → Each project gets its own daemon at `.beans/bd.sock` (LSP model)
...
cd ~/work/webapp && bd ready --json    # Uses ~/work/webapp/.beans/webapp.db

# Agent 2 working on API
cd ~/work/api && bd ready --json       # Uses ~/work/api/.beans/api.db
