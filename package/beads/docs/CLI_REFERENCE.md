...
# Example output:
# {
#   "database_path": "/path/to/.beans/beads.db",
#   "issue_prefix": "bd",
#   "daemon_running": true,
...
bd --db /path/to/.beans/beads.db <command>
...
**What gets removed:**
- `.beans/` directory (database, JSONL, config)
...
|-------|-------|---------|---------|
| Solid | Proto | `.beans/` | `bd formula list` |
| Liquid | Mol | `.beans/` | `bd mol pour` |
| Vapor | Wisp | `.beans/` (Ephemeral=true, not exported) | `bd mol wisp` |
...
bd import -i .beans/issues.jsonl --dry-run      # Preview changes
bd import -i .beans/issues.jsonl                # Import and update issues
bd import -i .beans/issues.jsonl --dedupe-after # Import + detect duplicates
