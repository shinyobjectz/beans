...
Beads now provides **enhanced Git worktree support** with a shared database architecture. All worktrees in a repository share the same `.beans` database located in the main repository, enabling seamless issue tracking across multiple working directories.
...
- Contains only the `.beans/` directory (sparse checkout)
...
├── .beans/                  # Shared database (main repo)
...
**Key points:**
- ✅ **One database** - All worktrees share the same `.beans` directory in main repo
...
1. **Priority search**: Main repository `.beans` directory first
...
4. **Validation**: Ensures `.beans` contains actual project files
...
**Solutions:**
```bash
# Ensure main repo has .beans directory
cd main-repo
ls -la .beans/
...
**Symptoms:** Warning about multiple `.beans` directories

**Solution:**
```bash
# bd shows warning with database locations
# Typically, the closest database (in main repo) is correct
# Remove extra .beans directories if they're not needed
```
...
export BEADS_DB=/path/to/specific/.beans/beads.db
```
...
```bash
cd ~/my-project
export BEADS_DIR=~/my-project-beads/.beans
...
# ~/bin/bd-myproj
#!/bin/bash
BEADS_DIR=~/my-project-beads/.beans exec bd "$@"
```
...
export BEADS_DIR=~/project-beads/.beans
