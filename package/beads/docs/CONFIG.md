...
3. Config file (`~/.config/bd/config.yaml` or `.beans/config.yaml`)
4. Defaults

### Config File Locations

Viper searches for `config.yaml` in these locations (in order):
1. `.beans/config.yaml` - Project-specific tool settings (version-controlled)
2. `~/.config/bd/config.yaml` - User-specific tool settings
3. `~/.beans/config.yaml` - Legacy user settings
...
`.beans/config.yaml` (project-specific):
```yaml
...
- **Per-project**: Isolated to each `.beans/*.db` database
...
4. **Security**: Store tokens in config, but add `.beans/*.db` to `.gitignore` (bd does this automatically)
