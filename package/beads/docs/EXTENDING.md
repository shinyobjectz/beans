...
> These tools use bd's CLI with `--json` flags or read/write the `.beans/` JSONL files directly.
>
...
// Open bd's storage
store, err := beads.NewSQLiteStorage(".beans/issues.db")
if err != nil {
    log.Fatal(err)
}
defer store.Close()
...
// Open bd's storage
store, err := sqlite.New(".beans/issues.db")
if err != nil {
    log.Fatal(err)
}
