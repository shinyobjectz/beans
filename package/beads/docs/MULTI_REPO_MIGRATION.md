...
By default, beads stores issues in `.beans/issues.jsonl` in your current repository. Multi-repo mode lets you:
...
{"id":"bd-xyz","source_repo":"~/.beans-planning","title":"Planning issue"}
```

- `.` = Current repository (default)
- `~/.beans-planning` = Contributor planning repo
...
bd create "Fix bug" -p 1  
# → Creates in ~/.beans-planning (source_repo = "~/.beans-planning")
```
...
# - Prompt you to create a planning repo (~/.beans-planning by default)
...
bd config set routing.contributor "~/.beans-planning"

# 3. Add planning repo to hydration sources
bd config set repos.additional "~/.beans-planning"
```
...
# Create planning issues (auto-routed to ~/.beans-planning)
...
# ✅ PR only contains code changes, no .beans/ pollution
```
...
mkdir -p ~/.beans-planning-personal
cd ~/.beans-planning-personal
git init
bd init --prefix exp

# 4. Configure multi-repo in team project
cd ~/projects/project
bd config set repos.additional "~/.beans-planning-personal"
```
...
bd create "Try alternative approach" -p 2 --repo ~/.beans-planning-personal
...
### Git merge conflicts in .beans/issues.jsonl
...
**Problem:** Your `~/.beans-planning` changes appear in PRs to upstream.

**Solution:** This shouldn't happen if configured correctly. Verify:
```bash
# Check that planning repo is separate from fork
ls -la ~/.beans-planning/.git  # Should exist
ls -la ~/projects/fork/.beans/  # Should NOT contain planning issues

# Verify routing
bd config get routing.contributor  # Should be ~/.beans-planning
```
...
# Before (single repo)
bd create "Issue" -p 1
# → Creates in .beans/issues.jsonl

# After (multi-repo configured)
bd create "Issue" -p 1
# → Auto-routed based on role
# → Old issues in .beans/issues.jsonl still work
```
...
### OSS Contributors
- ✅ Always use `~/.beans-planning` or similar for personal planning
- ✅ Never commit `.beans/` changes to upstream PRs
- ✅ Use descriptive prefixes (`plan-`, `exp-`) for clarity
- ❌ Don't mix planning and implementation in the same repo

### Teams
- ✅ Commit `.beans/issues.jsonl` to shared repository
- ✅ Use protected branch workflow for main/master
- ✅ Review issue changes in PRs like code changes
- ❌ Don't gitignore `.beans/` - you lose the git ledger
...
=======
- [bd-8rd](/.beans/issues.jsonl#bd-8rd) - Migration and onboarding epic
- [bd-mlcz](/.beans/issues.jsonl#bd-mlcz) - `bd migrate` command (planned)
- [bd-kla1](/.beans/issues.jsonl#bd-kla1) - `bd init --contributor` wizard ✅ implemented
- [bd-twlr](/.beans/issues.jsonl#bd-twlr) - `bd init --team` wizard ✅ implemented
>>>>>>> origin/bd-l0pg-slit
