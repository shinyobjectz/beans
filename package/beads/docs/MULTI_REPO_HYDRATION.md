...
CREATE TABLE repo_mtimes (
    ...
    jsonl_path TEXT NOT NULL,        -- Absolute path to .beans/issues.jsonl
...
- Use `.beans/config.yaml` instead of env vars (faster viper access)
...
2. Verify JSONL exists: `ls -la /path/to/repo/.beans/issues.jsonl`
...
- Or touch the JSONL file: `touch /path/to/repo/.beans/issues.jsonl`
