...
This creates a `.beans/` directory and configures beads to commit to `beads-sync` instead of `main`.
...
git add .beans/.gitignore .gitattributes
...
- `.beans/.gitignore` - Tells git what to ignore in .beans/ directory
...
- `.beans/beads.db` - SQLite database (local only, regenerated from JSONL)
- `.beans/daemon.lock`, `daemon.log`, `daemon.pid` - Runtime files
- `.beans/beads.left.jsonl`, `beads.right.jsonl` - Temporary merge artifacts

The sync branch (beads-sync) will contain:
- `.beans/issues.jsonl` - Issue data in JSONL format (committed automatically by daemon)
- `.beans/metadata.json` - Metadata about the beads installation
- `.beans/config.yaml` - Configuration template (optional)
...
│   └── beads-worktrees/
│       └── beads-sync/  # Worktree (only .beans/ checked out)
│           └── .beans/
│               └── issues.jsonl
├── .beans/                  # Your main copy
│   ├── beads.db
│   ├── issues.jsonl
│   └── .gitignore
...
- `.beans/.gitignore` - Tells git what to ignore
- `.gitattributes` - Merge driver configuration

Sync branch (beads-sync):
- `.beans/issues.jsonl` - Issue data (committed by daemon)
- `.beans/metadata.json` - Repository metadata
- `.beans/config.yaml` - Configuration template

Not tracked (gitignored):
- `.beans/beads.db` - SQLite database (local only)
- `.beans/daemon.*` - Runtime files

**Key points:**
- The worktree is in `.git/beads-worktrees/` (hidden from your workspace)
- Only `.beans/` is checked out in the worktree (sparse checkout)
...
1. Issue is updated in `.beans/beads.db` (SQLite database)
2. Daemon exports to `.beans/issues.jsonl` (JSONL file)
3. JSONL is copied to worktree (`.git/beads-worktrees/beads-sync/.beans/`)
...
This will:
- Create `.beans/` directory with database
...
To resolve:
1. Fix conflicts in .beans/issues.jsonl
2. git add .beans/issues.jsonl
3. git commit
...
1. Open `.beans/issues.jsonl` and look for conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`)
...
- Permission denied: Check `.beans/` directory permissions
...
- Sparse checkout means only `.beans/` is checked out
