...
The lock file is located at `.beans/.exclusive-lock` and contains JSON:
...
Check daemon logs (default: `.beans/daemon.log`) to troubleshoot lock issues.
...
2. **Create a lock**: Use your tool to create `.beans/.exclusive-lock`
3. **Verify daemon skips**: Check daemon logs for "Skipping database" message
4. **Release lock**: Remove `.beans/.exclusive-lock`
