...
- Routes to correct per-project daemon (`.beans/bd.sock`)
...
bd create "Fix bug" -p 1
# → Creates in planning repo (source_repo = "~/.beans-planning")
```
...
bd list --json | jq '.[] | select(.source_repo == "~/.beans-planning")'
```
...
# → Created in ~/.beans-planning (never appears in PRs)
...
# Git commit/push - no .beans/ pollution in PR ✅
```
...
bd create "Try alternative" -p 2 --repo ~/.beans-planning-personal
...
**Symptom:** `~/.beans-planning` changes appear in upstream PRs
...
### OSS Contributors
- ✅ Planning issues auto-route to `~/.beans-planning`
- ✅ Never commit `.beans/` in PRs to upstream
- ✅ Use `bd ready` to see all work (upstream + planning)
- ❌ Don't manually override routing without good reason

### Teams
- ✅ Commit `.beans/issues.jsonl` to shared repo
...
```bash
bd create "Issue" -p 1
# → Creates in .beans/issues.jsonl (single-repo mode)
```
...
bd config set routing.contributor "~/.beans-planning"  # Where contributor issues go
...
# Sample output:
{
  "database_path": "/Users/you/projects/myapp/.beans/beads.db",
  "config": {
    "routing": {
      "mode": "auto",
      "maintainer": ".",
      "contributor": "~/.beans-planning"
    },
    "repos": {
      "primary": ".",
