...
**The Problem:**
Git worktrees share the same `.git` directory and thus share the same `.beans` database. The daemon doesn't know which branch each worktree has checked out, which can cause it to commit/push to the wrong branch.
...
**Why It Matters:**
The daemon maintains its own view of the current working directory and git state. When multiple worktrees share the same `.beans` database, the daemon may commit changes intended for one branch to a different branch, leading to confusion and incorrect git history.
...
Create a `.beans/redirect` file pointing to the shared database location:

```bash
# In your secondary clone
mkdir -p .beans
echo "../main-clone/.beans" > .beans/redirect

# Or use an absolute path
echo "/path/to/shared/.beans" > .beans/redirect
```

The redirect file should contain a single path (relative or absolute) to the target `.beans` directory.
...
├── agent-1/
│   └── .beans/
│       └── redirect      ← Points to ../main-clone/.beans
└── agent-2/
    └── .beans/
        └── redirect      ← Points to ../main-clone/.beans
```
...
```bash
bd where
# /path/to/main-clone/.beans
#   (via redirect from /path/to/agent-1/.beans)
#   prefix: bd
#   database: /path/to/main-clone/.beans/beads.db
...
git checkout --theirs .beans/issues.jsonl  # Accept remote version
# OR
git checkout --ours .beans/issues.jsonl    # Keep local version
...
For Git merge conflicts in `.beans/issues.jsonl`, consider using **[beads-merge](https://github.com/neongreen/mono/tree/main/beads-merge)** - a specialized merge tool by @neongreen that:
...
- **Lightweight RPC server** - Runs at `.beans/bd.sock` in each project
...
- **Workspace routing** - Finds correct `.beans/bd.sock` based on working directory
