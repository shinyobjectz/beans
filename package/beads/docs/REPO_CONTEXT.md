...
different directories than where `.beans/` lives.
...
- A git worktree (separate working directory, shared `.beans/`)
...
|--------|----------|---------|
| `GitCmd()` | Git commands for beads operations | `git add .beans/`, `git push` |
...
// Use for: committing .beans/, pushing/pulling beads data
cmd := rc.GitCmd(ctx, "add", ".beans/issues.jsonl")
...
CWD is inside the repository containing `.beans/`:
...
User is in a worktree but `.beans/` lives in main repository:
...
$ bd sync
# GitCmd() runs in /project (main repo, where .beans lives)
```
...
| Field | Description |
|-------|-------------|
| `BeadsDir` | Actual `.beans/` directory (after following redirects) |
