...
rm .beans/*.db-journal .beans/*.db-wal .beans/*.db-shm
```
...
git filter-branch --force --index-filter \
  'git rm --cached --ignore-unmatch .beans/issues.jsonl' \
  --prune-empty -- --all
...
sqlite3 .beans/*.db "PRAGMA integrity_check;"

# If corrupted, reimport from JSONL (source of truth in git)
mv .beans/*.db .beans/*.db.backup
bd init
bd import -i .beans/issues.jsonl
```
...
If you see a warning about multiple `.beans` databases in the directory hierarchy:
...
║                                                                          ║
║ ▶ /path/to/project/.beans (15 issues)                                   ║
║   /path/to/parent/.beans (32 issues)                                    ║
║                                                                          ║
...
This means bd found multiple `.beans` directories in your directory hierarchy. The `▶` marker shows which database is actively being used (usually the closest one to your current directory).
...
1. **If you have nested projects** (intentional):
   ...
   - Set `BEADS_DIR` environment variable to point to your `.beans` directory if you want to override the default selection
...
2. **If you have accidental duplicates** (unintentional):
   ...
   - Remove the unwanted `.beans` directory: `rm -rf <unwanted-dir>/.beans`
   - Optionally import issues into the main database if needed

3. **Override database selection**:
   ```bash
   # Temporarily use specific .beans directory (recommended)
   BEADS_DIR=/path/to/.beans bd list

   # Or add to shell config for permanent override
   export BEADS_DIR=/path/to/.beans

   # Legacy method (deprecated, points to database file directly)
   BEADS_DB=/path/to/.beans/issues.db bd list
   export BEADS_DB=/path/to/.beans/issues.db
   ```
...
git add .beans/issues.jsonl
git commit
bd import -i .beans/issues.jsonl  # Sync to SQLite
```
...
If git shows a conflict in `.beans/issues.jsonl`, it's because the same issue was modified on both branches:

```bash
# Preview what will be updated
bd import -i .beans/issues.jsonl --dry-run

# Resolve git conflict (keep newer version or manually merge)
git checkout --theirs .beans/issues.jsonl  # Or --ours, or edit manually
...
bd export -o .beans/issues.jsonl
bd import -i .beans/issues.jsonl
...
bd export --format=jsonl --status=open -o .beans/issues.jsonl
...
If `.beans/issues.jsonl` is very large:

```bash
# Check file size
ls -lh .beans/issues.jsonl
...
bd --sandbox ready
bd --sandbox import -i .beans/issues.jsonl
```
...
bd import --force -i .beans/issues.jsonl
...
**Cause:** Windows Controlled Folder Access is blocking `bd.exe` from creating the `.beans` directory.

**Diagnosis:** Run with verbose flag to see the actual error:
```pwsh
bd init -v
# Error: failed to create .beans directory: mkdir .beans: The system cannot find the file specified
```
