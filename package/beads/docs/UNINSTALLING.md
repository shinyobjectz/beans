...
```bash
cp .beans/issues.jsonl ~/beads-backup-$(date +%Y%m%d).jsonl
```
...
If `.beans/` was tracked in git, commit its removal:
...
git config --get merge.beans.driver
...
This will recreate the `.beans/` directory, install hooks, and configure the merge driver.
