...
Git worktrees share the same `.git` directory and `.beans` database:
- All worktrees use the same `.beans/beads.db` file in the main repository
...
The daemon maintains its own view of the current working directory and git state. When multiple worktrees share the same `.beans` database, the daemon may commit changes intended for one branch to a different branch.
...
╠══════════════════════════════════════════════════════════════════════════╣
║ Git worktrees share the same .beans directory, which can cause the      ║
║ daemon to commit/push to the wrong branch.                               ║
║                                                                          ║
║ Shared database: /path/to/main/.beans                                    ║
...
**Database Discovery:**
- Searches main repository first for `.beans` directory
...
Git conflicts in `.beans/issues.jsonl` happen when:
...
# Resolve with: git checkout --theirs .beans/issues.jsonl
...
```bash
# After git merge creates conflict in .beans/issues.jsonl

# Option 1: Accept their version (remote)
git checkout --theirs .beans/issues.jsonl
bd import -i .beans/issues.jsonl

# Option 2: Keep our version (local)
git checkout --ours .beans/issues.jsonl
bd import -i .beans/issues.jsonl

# Option 3: Manual resolution in editor
# Edit .beans/issues.jsonl to remove conflict markers
bd import -i .beans/issues.jsonl
...
git config merge.beans.driver "bd merge %A %O %A %B"
git config merge.beans.name "bd JSONL merge driver"
...
git config merge.beans.driver "bd merge %A %O %A %B"
git config merge.beans.name "bd JSONL merge driver"
echo ".beans/issues.jsonl merge=beads" >> .gitattributes
```
...
Then resolve with:
```bash
jj resolve --tool=beads-merge .beans/issues.jsonl
```

**Note:** This only works for `.beans/issues.jsonl` since `bd merge` is a specialized JSONL merge tool.
...
- Supports shared mode via `--shared` flag (installs to `.beans-hooks/` instead)
...
.beans/*.jsonl text diff
```

This file is automatically created by `bd init` and is essential for:
- Preventing spurious merge conflicts in `.beans/issues.jsonl`
...
**Do NOT use Git LFS for `.beans/issues.jsonl`:**
...
**Symptoms:**
- Git merge always creates conflicts in `.beans/issues.jsonl`
- Merge driver not being used

**Solutions:**
```bash
# Check merge driver configured
git config merge.beans.driver
...
# Expected: .beans/issues.jsonl merge=beads
```
