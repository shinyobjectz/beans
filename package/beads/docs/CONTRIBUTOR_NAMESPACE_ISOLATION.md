...
work-tracking issues leak into PRs. The `.beans/issues.jsonl` file is intentionally
...
The PR diff includes their personal issues in `.beans/issues.jsonl`.
...
accordingly:
- **Maintainer** (SSH access): Issues go to `./.beans/` (project database)
- **Contributor** (HTTPS fork): Issues go to `~/.beans-planning/` (personal database)
...
2. **Routing Configuration** (`internal/config/config.go`):
   ...
   v.SetDefault("routing.contributor", "~/.beans-planning")
   ```
...
4. **Contributor Setup Wizard** (`cmd/bd/init_contributor.go`):
   ...
   bd init --contributor
   ```
   Creates `~/.beans-planning/` and configures routing.
...
1. **Actual Routing in `bd create`** (bd-6x6g):
   ...
   ```
   The routing is calculated but NOT used. Issues still go to `./.beans/`.
...
Options:
  1. Configure auto-routing (recommended)
     Creates ~/.beans-planning for personal tracking

  2. Continue to current repo
     Issue will appear in .beans/issues.jsonl (affects PRs)
...
func checkBeadsPollution(ctx context.Context) (CheckResult, error) {
    // Get git diff of .beans/issues.jsonl
    diff, err := gitDiff(".beans/issues.jsonl")
    if err != nil {
...
|-----------|--------------|---------------|-------|
| **Direct** | Uses `.beans/` directly | Uses `~/.beans-planning/.beans/` | Both use direct storage, no interaction |
...
1. **Separate databases**: Planning repo is completely independent - it has its own `.beans/` directory
...
# This configures:
# - Creates ~/.beans-planning/ with its own database
# - Sets routing.mode=auto
# - Sets routing.contributor=~/.beans-planning

# Verify
bd config get routing.mode        # → auto
bd config get routing.contributor # → ~/.beans-planning
```
...
BEADS_DIR=~/.beans-planning bd create "My task" -p 1

# Or per-shell session
export BEADS_DIR=~/.beans-planning
...
**Symptom**: Issues appear in `./.beans/issues.jsonl` instead of planning repo
...
**Diagnosis**:
```bash
ls -la ~/.beans-planning/.beans/  # Should exist
```
...
bd config set contributor.planning_repo ~/.beans-planning
...
1. **Source Repo Mismatch**: Issue has `source_repo != "."` but is in `./.beans/`
