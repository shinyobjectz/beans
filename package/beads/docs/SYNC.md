...
Beads uses an **exclusive file lock** (`.beans/.sync.lock`) to serialize sync operations:
...
| File | Purpose |
|------|---------|
| `.beans/issues.jsonl` | Current state (git-tracked) |
| `.beans/sync_base.jsonl` | Last-synced state (not tracked, per-machine) |
| `.beans/.sync.lock` | Concurrency guard (not tracked) |
| `.beans/beads.db` | SQLite database (not tracked) |
...
| `BEADS_DIR=<clone>/.beans` | Isolate database per clone |
