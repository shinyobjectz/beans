...
Append-only audit logging for agent interactions (prompts, responses, tool calls) in `.beans/interactions.jsonl`.
...
- `bd sync` includes `.beans/interactions.jsonl` in the commit allowlist (like `issues.jsonl`).
