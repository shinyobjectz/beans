...
|-------|------|---------|---------|----------|
| **Solid** | Proto | `.beans/` | Yes | Reusable template (epic with `template` label) |
| **Liquid** | Mol | `.beans/` | Yes | Persistent instance (real issues from template) |
| **Vapor** | Wisp | `.beans-wisp/` | No | Ephemeral instance (operational work, no audit trail) |
...
**"Wisp commands fail"**
- Wisps stored in `.beans-wisp/` (separate from `.beans/`)
