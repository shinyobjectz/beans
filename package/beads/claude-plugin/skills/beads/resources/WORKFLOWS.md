...
- Project has `.beans/` directory (project-local), OR
- `~/.beans/` exists (global fallback for any directory)
...
- [ ] If using global ~/.beans, note this in report
...
**Database selection**: bd auto-discovers which database to use (project-local `.beans/` takes precedence over global `~/.beans/`).
