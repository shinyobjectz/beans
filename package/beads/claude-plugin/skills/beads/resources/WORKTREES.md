...
All worktrees share one `.beans/` database via redirect files:

```
main-repo/
├── .beans/              ← Single source of truth
└── .worktrees/
    ├── feature-a/
    │   └── .beans       ← Redirect file (not directory)
    └── feature-b/
        └── .beans       ← Redirect file
```
...
```bash
bd where              # Shows actual .beans location (follows redirects)
