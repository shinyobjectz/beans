...
3. **Check JSONL file:**
   ```bash
   cat .beans/issues.jsonl | jq '.dependencies'
...
**BD Architecture:**
- **JSONL files** (`.beans/issues.jsonl`): Human-readable export format
- **SQLite database** (`.beans/*.db`): Source of truth for queries
...
- Version control of `.beans/*.jsonl` files
...
~/Google Drive/My Work/project/.beans/  # ✗ Will fail

# Correct location (local disk)
~/Repos/project/.beans/                 # ✓ Works reliably
~/Projects/project/.beans/              # ✓ Works reliably
```
...
**Alternative: Use global `~/.beans/` database**
...
bd create "My task"
# Uses ~/.beans/default.db (on local disk)
```
...
cat .beans/issues.jsonl | jq '.' | head -50
```
