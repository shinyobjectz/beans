...
func TestGetReadyWorkExternalDeps(t *testing.T) {
	...
	// Create .beans directory and config in external project
...
// but has no .beans directory (bd-mv6h).
func TestCheckExternalDepNoBeadsDirectory(t *testing.T) {
	ctx := context.Background()

	// Create a project directory WITHOUT .beans
	...
	if status.Satisfied {
		t.Error("Expected external dep to be unsatisfied when target has no .beans directory")
	}
...
func TestCheckExternalDepsBatching(t *testing.T) {
	...
	// Create .beans directory and config
	beadsDir := filepath.Join(externalDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
