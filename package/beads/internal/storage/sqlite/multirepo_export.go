...
func (s *SQLiteStorage) ExportToMultiRepo(ctx context.Context) (map[string]int, error) {
	...
	// Wisps exist only in SQLite and are shared via .beans/redirect, not JSONL.
...
func (s *SQLiteStorage) exportToRepo(ctx context.Context, repoPath string, issues []*types.Issue) (int, error) {
	...
	// For relative paths, resolve from repo root (parent of .beans/), NOT CWD.
	// This ensures paths like "oss/" in config become "{repo}/oss/", not ".beans/oss/"
	// when running from different directories or in daemon context.
	var absRepoPath string
	if filepath.IsAbs(expandedPath) {
		absRepoPath = expandedPath
	} else {
		// Resolve relative to repo root (parent of .beans/)
		// Config is at .beans/config.yaml, so go up twice
		...
		if configFile != "" {
			...
			// Fallback: dbPath is .beans/beads.db, go up one level to repo root
	...
	jsonlPath := filepath.Join(absRepoPath, ".beans", "issues.jsonl")

	// Ensure .beans directory exists
	beadsDir := filepath.Dir(jsonlPath)
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		return 0, fmt.Errorf("failed to create .beans directory: %w", err)
	}
