...
`
		...
		// Export - this should resolve "oss/" relative to tmpDir (repo root), not .beans/ (CWD)
		results, err := store.ExportToMultiRepo(ctx)
		if err != nil {
			t.Fatalf("ExportToMultiRepo() error = %v", err)
		}
		...
		// Verify the JSONL was written to the correct location (tmpDir/oss/.beans/issues.jsonl)
		// NOT to .beans/oss/.beans/issues.jsonl (which would happen with CWD-based resolution)
		expectedJSONL := filepath.Join(ossBeadsDir, "issues.jsonl")
		wrongJSONL := filepath.Join(beadsDir, "oss", ".beans", "issues.jsonl")
