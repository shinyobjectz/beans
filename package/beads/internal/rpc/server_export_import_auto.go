...
func (s *Server) checkAndAutoImportIfStale(req *Request) error {
	...
	workspaceRoot := filepath.Dir(dbDir) // Go up from .beans to workspace root
	if hasUncommittedBeadsFiles(workspaceRoot) {
		...
		fmt.Fprintf(os.Stderr, "Warning: auto-import skipped - .beans files have uncommitted changes. Run 'bd sync' after committing.\n")
		return nil
	}
...
// hasUncommittedBeadsFiles checks if .beans directory has uncommitted changes
...
func hasUncommittedBeadsFiles(workspacePath string) bool {
	...
	cmd := exec.CommandContext(ctx, "git", "status", "--porcelain", ".beans/")
	cmd.Dir = workspacePath
	output, err := cmd.Output()
	if err != nil {
		// If git command fails, assume not in git repo or .beans not tracked
