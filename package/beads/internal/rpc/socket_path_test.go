...
func TestShortSocketPath_ShortPath(t *testing.T) {
	// Short paths should use the natural .beans/bd.sock location
	workspacePath := "/tmp/myrepo"
	socketPath := ShortSocketPath(workspacePath)

	expected := filepath.Join(workspacePath, ".beans", "bd.sock")
...
func TestShortSocketPath_LongPath(t *testing.T) {
	...
	// Create a path that's definitely over 103 chars when .beans/bd.sock is added
...
func TestNeedsShortPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.want {
				naturalPath := filepath.Join(tt.workspace, ".beans", "bd.sock")
...
func TestShortSocketPath_EdgeCase_ExactLimit(t *testing.T) {
	...
	// .beans/bd.sock adds 15 characters
	...
	expected := filepath.Join(workspace, ".beans", "bd.sock")
	if socketPath != expected {
