...
//
// If the computed .beans/bd.sock path is short enough, it returns that directly.
...
func ShortSocketPath(workspacePath string) string {
	...
	// Compute the "natural" socket path in .beans/
	naturalPath := filepath.Join(workspacePath, ".beans", "bd.sock")
...
func EnsureSocketDir(socketPath string) (string, error) {
	...
	// Don't create .beans directories - those should exist
...
func CleanupSocketDir(socketPath string) error {
	...
	// For .beans/ directories, just remove the socket file
