...
func TestFindConfigYAMLPath(t *testing.T) {
	// Create temp dir with .beans/config.yaml
	tmpDir := t.TempDir()
	...
	// Verify path ends with .beans/config.yaml
	if filepath.Base(found) != "config.yaml" {
		t.Errorf("expected path ending with config.yaml, got %s", found)
	}
	if filepath.Base(filepath.Dir(found)) != ".beans" {
		t.Errorf("expected path in .beans dir, got %s", found)
	}
}
