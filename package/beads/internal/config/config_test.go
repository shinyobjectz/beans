...
`
	...
	// Move config to .beans directory
...
// are resolved from repo root (parent of .beans/), NOT from CWD.
...
func TestResolveExternalProjectPathFromRepoRoot(t *testing.T) {
	...
	t.Run("relative path resolved from repo root not CWD", func(t *testing.T) {
		...
		// Create .beans directory with config file
		beadsDir := filepath.Join(tmpDir, ".beans")
		if err := os.MkdirAll(beadsDir, 0750); err != nil {
...
`
		...
		// This simulates daemon context where CWD is .beans/
		...
		// NOT to .beans/beads-project (CWD + relative path)
	...
	t.Run("CWD should not affect resolution", func(t *testing.T) {
		...
		// Create main repo with .beans and target project
		mainRepoDir := filepath.Join(tmpDir, "main-repo")
		beadsDir := filepath.Join(mainRepoDir, ".beans")
		if err := os.MkdirAll(beadsDir, 0750); err != nil {
			t.Fatalf("failed to create .beans dir: %v", err)
		}
...
`
		...
		// because when CWD is tmpDir, the config file at mainRepoDir/.beans/config.yaml
		// won't be discovered (viper searches from CWD upward)
		testDirs := []string{
			mainRepoDir, // From repo root
			beadsDir,    // From .beans/ (daemon context)
		}
...
func TestRoutingModeDefaultIsEmpty(t *testing.T) {
	...
	// to prevent unexpected auto-routing to ~/.beans-planning
	// Isolate from environment variables
	restore := envSnapshot(t)
	defer restore()
	...
	if got := GetString("routing.contributor"); got != "~/.beans-planning" {
		t.Errorf("GetString(routing.contributor) = %q, want \"~/.beans-planning\"", got)
	}
}
...
func TestGetCustomTypesFromYAML(t *testing.T) {
	...
	// Create a temporary directory with a .beans/config.yaml
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
...
func TestGetCustomTypesFromYAML_NotSet(t *testing.T) {
	...
	// Create a temporary directory with a .beans/config.yaml WITHOUT types.custom
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatalf("failed to create .beans directory: %v", err)
