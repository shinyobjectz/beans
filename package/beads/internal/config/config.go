...
func Initialize() error {
	...
	// Precedence: project .beans/config.yaml > ~/.config/bd/config.yaml > ~/.beans/config.yaml
	configFileSet := false

	// 1. Walk up from CWD to find project .beans/config.yaml
	//    This allows commands to work from subdirectories
	cwd, err := os.Getwd()
	if err == nil && !configFileSet {
		// Walk up parent directories to find .beans/config.yaml
		for dir := cwd; dir != filepath.Dir(dir); dir = filepath.Dir(dir) {
			beadsDir := filepath.Join(dir, ".beans")
			configPath := filepath.Join(beadsDir, "config.yaml")
			if _, err := os.Stat(configPath); err == nil {
				// Found .beans/config.yaml - set it explicitly
	...
	// 3. Home directory (~/.beans/config.yaml)
	if !configFileSet {
		if homeDir, err := os.UserHomeDir(); err == nil {
			configPath := filepath.Join(homeDir, ".beans", "config.yaml")
			if _, err := os.Stat(configPath); err == nil {
				v.SetConfigFile(configPath)
				configFileSet = true
			}
...
func ResolveExternalProjectPath(projectName string) string {
	...
	// Resolve relative paths from repo root (parent of .beans/), NOT CWD.
	...
	if !filepath.IsAbs(path) {
		// Config is at .beans/config.yaml, so go up twice to get repo root
		configFile := ConfigFileUsed()
		if configFile != "" {
			repoRoot := filepath.Dir(filepath.Dir(configFile)) // .beans/config.yaml -> repo/
