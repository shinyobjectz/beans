...
// findProjectConfigYaml finds the project's .beans/config.yaml file.
func findProjectConfigYaml() (string, error) {
	...
	for dir := cwd; dir != filepath.Dir(dir); dir = filepath.Dir(dir) {
		configPath := filepath.Join(dir, ".beans", "config.yaml")
		if _, err := os.Stat(configPath); err == nil {
			return configPath, nil
		}
	}

	return "", fmt.Errorf("no .beans/config.yaml found (run 'bd init' first)")
}
