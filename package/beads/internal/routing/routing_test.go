...
// handles symlinked .beans directories by using findTownRootFromCWD() instead of
// walking up from the beadsDir path.
//
// Scenario: ~/gt/.beans is a symlink to ~/gt/olympus/.beans
// Before fix: walking up from ~/gt/olympus/.beans finds ~/gt/olympus (WRONG)
...
func TestFindTownRoutes_SymlinkedBeadsDir(t *testing.T) {
	...
	// Create olympus/.beans with routes.jsonl
	olympusBeadsDir := filepath.Join(tmpDir, "olympus", ".beans")
...
`
	...
	// Create daedalus/mayor/rig/.beans as target rig
	daedalusBeadsDir := filepath.Join(tmpDir, "daedalus", "mayor", "rig", ".beans")
	...
	// Create symlink: tmpDir/.beans -> olympus/.beans
	...
	expectedRigPath := filepath.Join(tmpDir, "daedalus", "mayor", "rig", ".beans")
	for _, route := range routes {
		if route.Prefix == "gt-" {
			actualPath := filepath.Join(townRoot, route.Path, ".beans")
