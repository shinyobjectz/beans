...
type Route struct {
	...
	Path   string `json:"path"`   // Relative path to .beans directory
}
...
// where routes.jsonl lives at ~/gt/.beans/ rather than in individual rig directories.
...
//   - currentBeadsDir: the current .beans directory (used to find routes.jsonl)
//
// Returns:
//   - beadsDir: the target .beans directory path
...
func ResolveBeadsDirForRig(rigOrPrefix, currentBeadsDir string) (beadsDir string, prefix string, err error) {
	...
	if route.Path == "." {
		...
		targetPath = filepath.Join(townRoot, ".beans")
	} else {
		// Normal path resolution relative to town root
		targetPath = filepath.Join(townRoot, route.Path, ".beans")
	}
...
//   - currentBeadsDir: the current/local .beans directory path
//
// Returns:
//   - beadsDir: the resolved .beans directory path
...
// This is used to handle symlinked .beans directories correctly.
// By starting from CWD instead of the beads directory path, we find the correct
// town root even when .beans is a symlink that points elsewhere.
...
// from <townRoot>/.beans/routes.jsonl.
// Returns (routes, townRoot). Returns nil routes if not in an orchestrator town or no routes found.
//
// IMPORTANT: This function handles symlinked .beans directories correctly.
// When .beans is a symlink (e.g., ~/gt/.beans -> ~/gt/olympus/.beans), we must
...
func findTownRoutes(currentBeadsDir string) ([]Route, string) {
	...
	if err == nil && len(routes) > 0 {
		...
		// We must NOT use currentBeadsDir as the starting point because if .beans
		// is a symlink (e.g., ~/gt/.beans -> ~/gt/olympus/.beans), currentBeadsDir
		// will be the resolved path (e.g., ~/gt/olympus/.beans) and walking up
	...
	townBeadsDir := filepath.Join(townRoot, ".beans")
