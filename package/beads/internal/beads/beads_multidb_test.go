...
func TestFindAllDatabases(t *testing.T) {
	// Create a temporary directory structure with multiple .beans databases
	...
	// Root .beans
	rootBeads := filepath.Join(tmpDir, ".beans")
	...
	project1Beads := filepath.Join(project1Dir, ".beans")
	...
	// Should find only the closest database (gt-bzd: stop searching when .beans found)
	// Parent .beans directories are out of scope in multi-workspace setups
	if len(databases) != 1 {
	...
	// Root database should NOT be found - it's out of scope (parent of closest .beans)
