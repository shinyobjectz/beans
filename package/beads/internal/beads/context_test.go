...
func TestGetRepoContextForWorkspace_NormalRepo(t *testing.T) {
	...
	// Create .beans directory with required files
...
func TestGetRepoContextForWorkspace_IgnoresBEADS_DIR(t *testing.T) {
	...
	for _, repo := range []string{repo1, repo2} {
		...
		beadsDir := filepath.Join(repo, ".beans")
		if err := os.MkdirAll(beadsDir, 0750); err != nil {
			t.Fatalf("failed to create .beans in %s: %v", repo, err)
		}
		if err := os.WriteFile(filepath.Join(beadsDir, "beads.db"), []byte{}, 0644); err != nil {
			t.Fatalf("failed to create beads.db: %v", err)
		}
	...
	// Set BEADS_DIR to repo2's .beans
	os.Setenv("BEADS_DIR", filepath.Join(repo2, ".beans"))

	// Get context for repo1 - should find repo1's .beans, NOT repo2's
	...
	expectedBeadsDir := resolveSymlinks(filepath.Join(repo1, ".beans"))
...
// TestGetRepoContextForWorkspace_MissingBeadsDir tests error when .beans doesn't exist
func TestGetRepoContextForWorkspace_MissingBeadsDir(t *testing.T) {
	...
	// No .beans directory created
	_, err := GetRepoContextForWorkspace(tmpDir)
	if err == nil {
		t.Error("expected error when .beans directory is missing")
	}
}
...
func TestRepoContext_Validate(t *testing.T) {
	...
	// Remove the .beans directory to make context stale
	if err := os.RemoveAll(beadsDir); err != nil {
		t.Fatalf("failed to remove .beans: %v", err)
	}
...
func TestRepoContext_Validate_RepoRootRemoved(t *testing.T) {
	...
	beadsDir := filepath.Join(repoDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0750); err != nil {
		t.Fatalf("failed to create .beans: %v", err)
	}
	if err := os.WriteFile(filepath.Join(beadsDir, "beads.db"), []byte{}, 0644); err != nil {
...
func TestIsPathInSafeBoundary(t *testing.T) {
	...
	tests := []struct {
		...
		{"system /root", "/root/.beans", false},
		{"system /bin", "/bin/.beans", false},
		{"system /sbin", "/sbin/.beans", false},
		{"system /opt", "/opt/beads", false},
		{"macOS /System", "/System/Library/.beans", false},
		{"macOS /Library", "/Library/Application Support/.beans", false},
		{"macOS /private", "/private/etc/.beans", false},

		// Safe paths - should be accepted
		{"user home directory", filepath.Join(homeDir, "projects/.beans"), true},
...
func TestGetRepoContextForWorkspace_RedirectToUnsafeLocation(t *testing.T) {
	...
	// Create .beans directory with a redirect file pointing outside safe boundary
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0750); err != nil {
		t.Fatalf("failed to create .beans dir: %v", err)
	}
	...
	// Note: FollowRedirect won't follow non-existent paths, but if /etc/.beans existed
	// and contained beads files, this security check would catch it
	redirectFile := filepath.Join(beadsDir, "redirect")
	if err := os.WriteFile(redirectFile, []byte("/etc/.beans\n"), 0644); err != nil {
		t.Fatalf("failed to write redirect file: %v", err)
	}

	// Since /etc/.beans doesn't exist, FollowRedirect returns original path
	// So create a valid beads.db in the local .beans to get past initial validation,
	...
	// (because /etc/.beans doesn't exist and FollowRedirect fell back)
...
func TestGetRepoContextForWorkspace_RedirectWithinRepo(t *testing.T) {
	...
	// Create .beans with actual files in repo2
	beadsDir2 := filepath.Join(repo2, ".beans")
	if err := os.MkdirAll(beadsDir2, 0750); err != nil {
		t.Fatalf("failed to create .beans in repo2: %v", err)
	}
	if err := os.WriteFile(filepath.Join(beadsDir2, "beads.db"), []byte{}, 0644); err != nil {
		t.Fatalf("failed to create beads.db in repo2: %v", err)
	}

	// Create .beans in repo1 with redirect to repo2
	beadsDir1 := filepath.Join(repo1, ".beans")
	if err := os.MkdirAll(beadsDir1, 0750); err != nil {
		t.Fatalf("failed to create .beans in repo1: %v", err)
	}
	...
	// Note: GetRepoContextForWorkspace ignores BEADS_DIR and looks for .beans in workspace
	// But it does follow the redirect file in the local .beans
	rc, err := GetRepoContextForWorkspace(repo1)
	if err != nil {
		t.Fatalf("GetRepoContextForWorkspace failed for safe redirect: %v", err)
	}

	// Verify the redirect was followed to repo2's .beans
