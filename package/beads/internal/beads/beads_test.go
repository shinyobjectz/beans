...
func TestFindDatabasePathInTree(t *testing.T) {
	...
	// Create .beans directory with a database file
	...
	// Set BEADS_DIR to our test .beans directory to override git repo detection
...
func TestFindDatabasePathNotFound(t *testing.T) {
	...
	// Create temporary directory without .beans
	tmpDir, err := os.MkdirTemp("", "beads-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
...
func TestFindBeadsDirSkipsDaemonRegistry(t *testing.T) {
	...
	// Create .beans with only daemon registry files (should be skipped)
	beadsDir := filepath.Join(tmpDir, ".beans")
...
func TestFindDatabasePathHomeDefault(t *testing.T) {
	...
	// If result is not empty, verify it contains .beans
...
func TestFollowRedirect(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func(t *testing.T, tmpDir string) (string, string) {
				// Create stub .beans with redirect
				stubDir := filepath.Join(tmpDir, "project", ".beans")
				if err := os.MkdirAll(stubDir, 0755); err != nil {
					t.Fatal(err)
				}

				// Create target .beans directory
				targetDir := filepath.Join(tmpDir, "actual", ".beans")
				...
				if err := os.WriteFile(redirectPath, []byte("../actual/.beans\n"), 0644); err != nil {
					t.Fatal(err)
				}
		...
		},
		{
			...
			setupFunc: func(t *testing.T, tmpDir string) (string, string) {
				...
				redirectPath := filepath.Join(stubDir, "redirect")
				if err := os.WriteFile(redirectPath, []byte("/nonexistent/path/.beans\n"), 0644); err != nil {
					t.Fatal(err)
				}
...
func TestFindDatabasePathWithRedirect(t *testing.T) {
	...
	// Create target .beans directory with actual database
	targetDir := filepath.Join(tmpDir, "actual", ".beans")
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		t.Fatal(err)
	}
...
func TestFindBeadsDirWithRedirect(t *testing.T) {
	...
	// Create target .beans directory with project files
	targetDir := filepath.Join(tmpDir, "actual", ".beans")
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		t.Fatal(err)
	}
...
// TestFindBeadsDir_Worktree tests that FindBeadsDir correctly finds the .beans
...
func TestFindBeadsDir_Worktree(t *testing.T) {
	...
	// Create .beans directory in main repo with a database
	mainBeadsDir := filepath.Join(mainRepoDir, ".beans")
	...
	// Create .beans directory in worktree with its own database
	worktreeBeadsDir := filepath.Join(worktreeDir, ".beans")
	...
	// FindBeadsDir should prioritize the main repo's .beans for worktrees (bd-de6)
	result := FindBeadsDir()
	...
	if resultResolved != mainBeadsDirResolved {
		t.Errorf("FindBeadsDir() = %q, want main repo .beans %q (prioritized for worktrees)", result, mainBeadsDir)
	}

	// Verify we're NOT finding the worktree's .beans (should fall back only if main repo has no .beans)
	if resultResolved == worktreeBeadsDirResolved {
		t.Errorf("FindBeadsDir() returned worktree .beans %q instead of main repo .beans %q - prioritization not working!", worktreeBeadsDir, mainBeadsDir)
	}
}
...
// key test for bd-745 - worktrees should share the same .beans database.
func TestFindDatabasePath_Worktree(t *testing.T) {
	...
	// Create .beans directory in main repo with database
	mainBeadsDir := filepath.Join(mainRepoDir, ".beans")
	if err := os.MkdirAll(mainBeadsDir, 0755); err != nil {
		t.Fatal(err)
	}
...
// its own .beans directory, FindDatabasePath finds the shared database in the main
...
func TestFindDatabasePath_WorktreeNoLocalDB(t *testing.T) {
	...
	// Create a worktree WITHOUT a .beans directory
	worktreeDir := filepath.Join(tmpDir, "worktree")
	cmd = exec.Command("git", "worktree", "add", worktreeDir, "HEAD")
	cmd.Dir = mainRepoDir
	if err := cmd.Run(); err != nil {
	...
	// Note: We do NOT create .beans in the worktree
	// The worktree got .beans from the commit, so we need to remove it
	worktreeBeadsDir := filepath.Join(worktreeDir, ".beans")
