...
func setupClone(t *testing.T, tmpDir, remoteDir, name, bdPath string) string {
	...
	if name == "A" {
		...
		runCmd(t, cloneDir, "git", "add", ".beans")
...
func resolveConflictMarkersIfPresent(t *testing.T, cloneDir string) {
	t.Helper()
	jsonlPath := filepath.Join(cloneDir, ".beans", "issues.jsonl")
	...
	if strings.Contains(string(jsonlContent), "<<<<<<<") {
		...
		runCmd(t, cloneDir, "git", "add", ".beans/issues.jsonl")
...
%s --no-daemon export -o .beans/issues.jsonl >/dev/null 2>&1 || true
git add .beans/issues.jsonl >/dev/null 2>&1 || true
exit 0
`, bdCmd)
	postMerge := fmt.Sprintf(`#!/bin/sh
%s --no-daemon import -i .beans/issues.jsonl >/dev/null 2>&1 || true
exit 0
`, bdCmd)
