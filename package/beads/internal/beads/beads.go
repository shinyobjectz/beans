...
// DataDirName is the name of the data directory (default: .beans for BEANS, was .beans)
const DataDirName = ".beans"

// RedirectFileName is the name of the file that redirects to another data directory
...
// FollowRedirect checks if a .beans directory contains a redirect file and follows it.
// If a redirect file exists, it returns the target .beans directory path.
...
// .beans directory. Relative paths are resolved from the parent directory of the
// original .beans directory (i.e., the project root).
//
...
func FollowRedirect(beadsDir string) string {
	...
	// Resolve relative paths from the parent of the .beans directory (project root)
...
type RedirectInfo struct {
	// IsRedirected is true if the local .beans has a redirect file
	IsRedirected bool
	// LocalDir is the local .beans directory (the one with the redirect file)
	LocalDir string
	// TargetDir is the actual .beans directory being used (after following redirect)
	TargetDir string
}

// GetRedirectInfo checks if the current beads directory is redirected.
// It searches for the local .beans/ directory and checks if it contains a redirect file.
// Returns RedirectInfo with IsRedirected=true if a redirect is active.
//
// bd-wayc3: This function now also checks the git repo's local .beans directory even when
...
func GetRedirectInfo() RedirectInfo {
	// First, always check the git repo's local .beans directory for redirects
...
// findLocalBdsDirInRepo finds the .beans directory relative to the git repo root.
// This ignores BEADS_DIR to find the "true local" .beans for redirect detection.
...
func findLocalBdsDirInRepo() string {
	...
	beadsDir := filepath.Join(repoRoot, ".beans")
...
// findLocalBeadsDir finds the local .beans directory without following redirects.
...
func findLocalBeadsDir() string {
	...
	for dir := cwd; dir != "/" && dir != "."; {
		beadsDir := filepath.Join(dir, ".beans")
		if info, err := os.Stat(beadsDir); err == nil && info.IsDir() {
			return beadsDir
		}
...
// findDatabaseInBeadsDir searches for a database file within a .beans directory.
...
//  1. $BEADS_DIR environment variable (points to .beans directory)
//  2. $BEADS_DB environment variable (points directly to database file, deprecated)
//  3. .beans/*.db in current directory or ancestors
//
// Redirect files are supported: if a .beans/redirect file exists, its contents
// are used as the actual .beans directory path.
//
// Returns empty string if no database is found.
func FindDatabasePath() string {
	// 1. Check BEADS_DIR environment variable (preferred)
	if beadsDir := os.Getenv("BEADS_DIR"); beadsDir != "" {
		// Canonicalize the path to prevent nested .beans directories
	...
	// 3. Search for .beans/*.db in current directory and ancestors
	if foundDB := findDatabaseInTree(); foundDB != "" {
		return utils.CanonicalizePath(foundDB)
	}

	// No fallback to ~/.beans - return empty string
	return ""
}

// hasBeadsProjectFiles checks if a .beans directory contains actual project files.
...
// This prevents FindBeadsDir from returning ~/.beans/ which only has registry.json.
...
// FindBeadsDir finds the .beans/ directory in the current directory tree
...
// Redirect files are supported: if a .beans/redirect file exists, its contents
// are used as the actual .beans directory path.
// For worktrees, prioritizes the main repository's .beans directory.
...
func FindBeadsDir() string {
	...
	// 3. Search for .beans/ in current directory and ancestors
	cwd, err := os.Getwd()
	if err != nil {
		return ""
	}
...
type DatabaseInfo struct {
	...
	BeadsDir   string // Parent .beans directory
...
// findDatabaseInTree walks up the directory tree looking for .beans/*.db
...
// FindAllDatabases scans the directory hierarchy for the closest .beans directory.
// Returns a slice with at most one DatabaseInfo - the closest database to CWD.
// Stops searching upward as soon as a .beans directory is found,
// because in multi-workspace setups, nested .beans directories
...
func FindAllDatabases() []DatabaseInfo {
	...
	for {
		...
		if info, err := os.Stat(beadsDir); err == nil && info.IsDir() {
			...
			// Found .beans/ directory, look for *.db files
			matches, err := filepath.Glob(filepath.Join(beadsDir, "*.db"))
			if err == nil && len(matches) > 0 {
				dbPath := matches[0]
				...
				// Stop searching upward - the closest .beans is the one to use
