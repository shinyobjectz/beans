...
func TestFindAllDatabases_SymlinkDeduplication(t *testing.T) {
	...
	// Create real directory with .beans database
	realDir := filepath.Join(tmpDir, "real")
	if err := os.MkdirAll(realDir, 0750); err != nil {
		t.Fatal(err)
	}

	beadsDir := filepath.Join(realDir, ".beans")
	...
	// - Once via symlink_to_real/.beans/test.db
	// - Once via real/.beans/test.db (when walking up to parent)
	if len(databases) != 1 {
