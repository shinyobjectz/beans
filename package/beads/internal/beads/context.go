...
//   - RepoRoot: where .beans/ lives (for git operations on beads data)
...
type RepoContext struct {
	// BeadsDir is the actual .beans directory path (after following redirects).
	BeadsDir string
...
//
// Returns an error if no .beans directory can be found.
...
func buildRepoContext() (*RepoContext, error) {
	// 1. Find .beans directory (respects BEADS_DIR env var)
	beadsDir := FindBeadsDir()
	if beadsDir == "" {
		return nil, fmt.Errorf("no .beans directory found")
	}
...
// Pattern:
//
//	cmd := rc.GitCmd(ctx, "add", ".beans/")
//	output, err := cmd.Output()
//
// Equivalent to running: cd $RepoRoot && git add .beans/
...
func buildRepoContextForWorkspace(workspacePath string) (*RepoContext, error) {
	...
	// 2. Find .beans directory in the appropriate location
	beadsDir := filepath.Join(repoRoot, ".beans")

	// Check if .beans exists
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		return nil, fmt.Errorf("no .beans directory found at %s", beadsDir)
	}
