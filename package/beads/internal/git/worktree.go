...
func (wm *WorktreeManager) CreateBeadsWorktree(branch, worktreePath string) error {
	...
	// Configure sparse checkout to only include .beans/
...
func (wm *WorktreeManager) SyncJSONLToWorktreeWithOptions(worktreePath, jsonlRelPath string, opts SyncOptions) error {
	...
	// worktree name (e.g., "main/.beans/issues.jsonl"). The sync branch uses
	// sparse checkout for .beans/* so we normalize to strip leading components.
...
// configureSparseCheckout sets up sparse checkout to only include .beans/
...
func (wm *WorktreeManager) configureSparseCheckout(worktreePath string) error {
	...
	// Set sparse checkout to only include .beans/
	setCmd := exec.Command("git", "sparse-checkout", "set", "/.beans/")
...
func (wm *WorktreeManager) verifySparseCheckout(worktreePath string) error {
	...
	// Verify it contains .beans
	if !strings.Contains(string(output), ".beans") {
		return fmt.Errorf("sparse-checkout does not include .beans")
	}
