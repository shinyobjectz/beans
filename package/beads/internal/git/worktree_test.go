...
func setupTestRepo(t *testing.T) (repoPath string, cleanup func()) {
	...
	// Create .beans directory and a test file
	...
	// Create a file outside .beans to test sparse checkout
...
func TestCreateBeadsWorktree(t *testing.T) {
	...
	t.Run("creates new branch worktree", func(t *testing.T) {
		...
		// Verify .beans directory exists in worktree
		beadsDir := filepath.Join(worktreePath, ".beans")
		if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
			t.Errorf(".beans directory not found in worktree")
		}
...
func TestSyncJSONLToWorktree(t *testing.T) {
	...
	mainJSONL := filepath.Join(repoPath, ".beans", "test.jsonl")
	...
	// Sync to worktree
	if err := wm.SyncJSONLToWorktree(worktreePath, ".beans/test.jsonl"); err != nil {
		t.Fatalf("SyncJSONLToWorktree failed: %v", err)
	}

	// Verify the data was synced
	worktreeJSONL := filepath.Join(worktreePath, ".beans", "test.jsonl")
...
func TestSparseCheckoutConfiguration(t *testing.T) {
	...
	t.Run("sparse checkout includes .beans", func(t *testing.T) {
...
func TestSyncJSONLToWorktreeErrors(t *testing.T) {
	...
	t.Run("fails when source file does not exist", func(t *testing.T) {
		err := wm.SyncJSONLToWorktree(worktreePath, ".beans/nonexistent.jsonl")
		if err == nil {
...
func TestCreateBeadsWorktreeWithExistingBranch(t *testing.T) {
	...
	beadsDir := filepath.Join(worktreePath, ".beans")
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		t.Error(".beans directory not found in worktree")
	}
}
...
func TestSyncJSONLToWorktreeMerge(t *testing.T) {
	...
	t.Run("merges when worktree has more issues than local", func(t *testing.T) {
		...
		worktreeJSONL := filepath.Join(worktreePath, ".beans", "issues.jsonl")
...
`
		...
		mainJSONL := filepath.Join(repoPath, ".beans", "issues.jsonl")
...
`
		...
		if err := wm.SyncJSONLToWorktree(worktreePath, ".beans/issues.jsonl"); err != nil {
			t.Fatalf("SyncJSONLToWorktree failed: %v", err)
		}
...
`
		...
		if err := wm.SyncJSONLToWorktreeWithOptions(worktreePath, ".beans/issues.jsonl", SyncOptions{ForceOverwrite: true}); err != nil {
...
`
		...
		if err := wm.SyncJSONLToWorktreeWithOptions(worktreePath, ".beans/issues.jsonl", SyncOptions{ForceOverwrite: false}); err != nil {
			t.Fatalf("SyncJSONLToWorktreeWithOptions failed: %v", err)
		}
...
func TestCreateBeadsWorktree_MainRepoSparseCheckoutDisabled(t *testing.T) {
	...
	// Check that .beans exists but other.txt does not
	beadsDir := filepath.Join(worktreePath, ".beans")
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		t.Error(".beans directory should exist in worktree (sparse checkout should include it)")
	}
...
func TestNormalizeBeadsRelPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:    ".beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
			name:     "worktree prefix - main",
			input:    "main/.beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
			name:     "worktree prefix - feature branch",
			input:    "feat-tts-config/.beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
			name:     "nested worktree prefix",
			input:    "worktrees/feature/.beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
			name:     "metadata file",
			input:    "main/.beans/metadata.json",
			expected: ".beans/metadata.json",
		},
		{
			name:     "no .beans in path",
			input:    "some/other/path.jsonl",
			expected: "some/other/path.jsonl",
		},
		{
			name:     "only .beans dir (no trailing slash - not normalized)",
			input:    ".beans",
			expected: ".beans",
		},
		{
