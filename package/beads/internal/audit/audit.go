...
const (
	// FileName is the audit log file name stored under .beans/.
...
func Path() (string, error) {
	...
	if beadsDir == "" {
		return "", fmt.Errorf("no .beans directory found")
	}
	return filepath.Join(beadsDir, FileName), nil
}

// EnsureFile creates .beans/interactions.jsonl if it does not exist.
...
// Append appends an event to .beans/interactions.jsonl as a single JSON line.
