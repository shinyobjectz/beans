...
func walkWithDepth(root string, currentDepth, maxDepth int, fn func(path string, info os.FileInfo) error) error {
	...
	for _, entry := range entries {
		...
		if info.IsDir() {
			name := entry.Name()
			if strings.HasPrefix(name, ".") && name != ".beans" {
				continue // Skip hidden dirs except .beans
			}
...
func discoverDaemonsLegacy(searchRoots []string) ([]DaemonInfo, error) {
	...
	// Search for .beans/bd.sock files (limit depth to avoid traversing entire filesystem)
...
func FindDaemonByWorkspace(workspacePath string) (*DaemonInfo, error) {
	// Determine the correct .beans directory location
	...
	mainWorkspace := filepath.Dir(beadsDir) // Get workspace from .beans dir
	...
	// Try legacy socket path in .beans directory (backwards compatibility)
...
// findBeadsDirForWorkspace determines the correct .beans directory for a workspace.
...
func findBeadsDirForWorkspace(workspacePath string) string {
	...
	if err != nil {
		...
		return filepath.Join(workspacePath, ".beans")
	}
	return rc.BeadsDir
}
...
// that need more than just the .beans directory path.
//
...
// checkDaemonErrorFile checks for a daemon-error file in the .beans directory
func checkDaemonErrorFile(socketPath string) string {
	// Socket path is typically .beans/bd.sock, so get the parent dir
	beadsDir := filepath.Dir(socketPath)
