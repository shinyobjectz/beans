...
func TestRegistryBasics(t *testing.T) {
	...
	tmpDir := t.TempDir()
	registryPath := filepath.Join(tmpDir, ".beans", "registry.json")
	...
	entry := RegistryEntry{
		...
		SocketPath:    "/test/workspace/.beans/bd.sock",
		DatabasePath:  "/test/workspace/.beans/beads.db",
		PID:           12345,
		Version:       "0.19.0",
		StartedAt:     time.Now(),
	}
...
func TestRegistryMultipleDaemons(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		entry := RegistryEntry{
			...
			SocketPath:    filepath.Join("/test", "workspace", string(rune('a'+i-1)), ".beans/bd.sock"),
			DatabasePath:  filepath.Join("/test", "workspace", string(rune('a'+i-1)), ".beans/beads.db"),
			PID:           10000 + i,
			Version:       "0.19.0",
			StartedAt:     time.Now(),
		}
...
func TestRegistryPathNormalization(t *testing.T) {
	...
	entry := RegistryEntry{
		...
		SocketPath:    filepath.Join(symlinkPath, ".beans/bd.sock"),
		DatabasePath:  filepath.Join(symlinkPath, ".beans/beads.db"),
		PID:           12345,
		Version:       "0.43.0",
		StartedAt:     time.Now(),
	}
	...
	entry2 := RegistryEntry{
		...
		SocketPath:    filepath.Join(realPath, ".beans/bd.sock"),
		DatabasePath:  filepath.Join(realPath, ".beans/beads.db"),
		PID:           54321,
		Version:       "0.43.0",
		StartedAt:     time.Now(),
	}
