...
func TestWalkWithDepth_SkipsHiddenDirs(t *testing.T) {
	...
	// Create .beans directory (should NOT skip)
	os.MkdirAll(filepath.Join(tmpDir, ".beans"), 0755)
	...
	os.WriteFile(filepath.Join(tmpDir, ".beans", "beads.db"), []byte("test"), 0644)
	...
	// Should only find beads.db from .beans directory
...
func TestFindBeadsDirForWorkspace_RegularRepo(t *testing.T) {
	...
	expected := filepath.Join(tmpDir, ".beans")
...
func TestFindBeadsDirForWorkspace_NonexistentDir(t *testing.T) {
	...
	expected := filepath.Join(nonexistent, ".beans")
	if beadsDir != expected {
		t.Errorf("Expected %s, got %s", expected, beadsDir)
	}
}
...
func TestDiscoverDaemons_LegacyWithTempDir(t *testing.T) {
	...
	// Create a fake .beans directory (no daemon running)
	beadsDir := filepath.Join(tmpDir, ".beans")
	os.MkdirAll(beadsDir, 0755)
...
func TestDiscoverDaemonsLegacy_MultipleRoots(t *testing.T) {
	...
	beadsDir1 := filepath.Join(tmpDir1, ".beans")
	beadsDir2 := filepath.Join(tmpDir2, ".beans")
