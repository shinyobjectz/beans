...
//  2. Town-level: $GT_ROOT/.beans/molecules.jsonl (if orchestrator detected via GT_ROOT)
//  3. User-level: ~/.beans/molecules.jsonl
//  4. Project-level: .beans/molecules.jsonl in the current project
//
// # Key Properties
//
...
func (l *Loader) LoadAll(ctx context.Context, beadsDir string) (*LoadResult, error) {
	...
	// 2. Load town-level molecules ($GT_ROOT/.beans/molecules.jsonl)
	...
	// 3. Load user-level molecules (~/.beans/molecules.jsonl)
	...
	// 4. Load project-level molecules (.beans/molecules.jsonl)
	if beadsDir != "" {
...
func getTownMoleculesPath() string {
	...
	gtPath := filepath.Join(gtRoot, ".beans", MoleculeFileName)
...
func getUserMoleculesPath() string {
	...
	userPath := filepath.Join(homeDir, ".beans", MoleculeFileName)
