...
func TestNormalizeBeadsRelPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "worktrees/feature-branch/.beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
		...
		{
			name:     "path with no .beans unchanged",
			input:    "some/other/path.txt",
			expected: "some/other/path.txt",
		},
		{
			name:     ".beans at start unchanged",
			input:    ".beans/subdir/file.jsonl",
			expected: ".beans/subdir/file.jsonl",
		},
		{
			name:     "similar prefix like .beans-backup not matched",
			input:    "foo/.beans-backup/.beans/issues.jsonl",
			expected: ".beans/issues.jsonl",
		},
		{
			name:     "only .beans-backup no real .beans unchanged",
			input:    "foo/.beans-backup/file.txt",
			expected: "foo/.beans-backup/file.txt",
		},
	}
