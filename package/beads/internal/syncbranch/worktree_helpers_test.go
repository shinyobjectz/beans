...
func TestHasChangesInWorktree(t *testing.T) {
	...
	t.Run("no changes in clean worktree", func(t *testing.T) {
		...
		jsonlPath := filepath.Join(repoDir, ".beans", "issues.jsonl")
	...
	t.Run("detects new untracked files", func(t *testing.T) {
		...
		// Add new file in .beans
		writeFile(t, filepath.Join(repoDir, ".beans", "metadata.json"), `{}`)
	...
	t.Run("handles file outside .beans dir", func(t *testing.T) {
		repoDir := setupTestRepo(t)
		defer os.RemoveAll(repoDir)

		// Create initial commit
		jsonlPath := filepath.Join(repoDir, "issues.jsonl") // Not in .beans
		writeFile(t, jsonlPath, `{"id":"test-1"}`)
		runGit(t, repoDir, "add", ".")
		runGit(t, repoDir, "commit", "-m", "initial")
		...
		if !hasChanges {
			t.Error("hasChangesInWorktree() = false with modified file outside .beans, want true")
		}
	})
}
...
func TestCommitInWorktree(t *testing.T) {
	...
	t.Run("commits staged changes", func(t *testing.T) {
		...
		err := commitInWorktree(ctx, repoDir, ".beans/issues.jsonl", "test commit message")
		if err != nil {
	...
	t.Run("commits entire .beans directory", func(t *testing.T) {
		repoDir := setupTestRepo(t)
		defer os.RemoveAll(repoDir)
		...
		// Add multiple files
		writeFile(t, filepath.Join(repoDir, ".beans", "metadata.json"), `{"version":"1"}`)
		writeFile(t, jsonlPath, `{"id":"test-1"}`+"\n"+`{"id":"test-2"}`)

		err := commitInWorktree(ctx, repoDir, ".beans/issues.jsonl", "multi-file commit")
		if err != nil {
			t.Fatalf("commitInWorktree() error = %v", err)
		}
...
func TestCopyJSONLToMainRepo(t *testing.T) {
	t.Run("copies JSONL file successfully", func(t *testing.T) {
		...
		// Create .beans directories
		os.MkdirAll(filepath.Join(worktreeDir, ".beans"), 0750)
		os.MkdirAll(filepath.Join(mainRepoDir, ".beans"), 0750)

		// Write content to worktree JSONL
		worktreeContent := `{"id":"test-1","title":"Test Issue"}`
		if err := os.WriteFile(filepath.Join(worktreeDir, ".beans", "issues.jsonl"), []byte(worktreeContent), 0600); err != nil {
			t.Fatalf("Failed to write worktree JSONL: %v", err)
		}

		mainJSONLPath := filepath.Join(mainRepoDir, ".beans", "issues.jsonl")

		err := copyJSONLToMainRepo(worktreeDir, ".beans/issues.jsonl", mainJSONLPath)
		if err != nil {
	...
	t.Run("also copies metadata.json if present", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(worktreeDir, ".beans", "issues.jsonl"), []byte(`{}`), 0600); err != nil {
			t.Fatalf("Failed to write worktree JSONL: %v", err)
		}
		metadataContent := `{"prefix":"bd"}`
		if err := os.WriteFile(filepath.Join(worktreeDir, ".beans", "metadata.json"), []byte(metadataContent), 0600); err != nil {
		...
		metadata, err := os.ReadFile(filepath.Join(mainRepoDir, ".beans", "metadata.json"))
		if err != nil {
...
func TestGetRemoteForBranch(t *testing.T) {
	...
	t.Run("returns origin as default", func(t *testing.T) {
		...
		// Create initial commit
		writeFile(t, filepath.Join(repoDir, ".beans", "issues.jsonl"), `{"id":"test-1"}`)
		runGit(t, repoDir, "add", ".")
		runGit(t, repoDir, "commit", "-m", "initial")
...
func TestGetRepoRoot(t *testing.T) {
	...
	t.Run("handles worktree correctly", func(t *testing.T) {
		...
		// Create initial commit
		writeFile(t, filepath.Join(mainRepoDir, ".beans", "issues.jsonl"), `{"id":"test-1"}`)
