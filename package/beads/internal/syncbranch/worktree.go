...
func copyJSONLToMainRepo(worktreePath, jsonlRelPath, jsonlPath string) error {
	...
	// worktree name (e.g., "main/.beans/issues.jsonl" instead of ".beans/issues.jsonl").
	// The sync branch uses sparse checkout for .beans/* so we normalize the path
	// to strip any leading components before .beans.
	normalizedRelPath := normalizeBeadsRelPath(jsonlRelPath)
...
func hasChangesInWorktree(ctx context.Context, worktreePath, filePath string) (bool, error) {
	// Check the entire .beans directory for changes
...
func commitInWorktree(ctx context.Context, worktreePath, jsonlRelPath, message string) error {
	// Stage the entire .beans directory
	beadsRelDir := filepath.Dir(jsonlRelPath)

	// Use -f (force) to add files even if they're gitignored
	// In contributor mode, .beans/ is excluded in .git/info/exclude but needs to be tracked in sync branch
...
func contentMergeRecovery(ctx context.Context, worktreePath, branch, remote string) error {
	// The JSONL is always at .beans/issues.jsonl relative to worktree
	jsonlRelPath := filepath.Join(".beans", "issues.jsonl")
...
// normalizeBeadsRelPath strips any leading path components before .beans/.
// This handles bare repo worktrees where the relative path includes the worktree
// name (e.g., "main/.beans/issues.jsonl" -> ".beans/issues.jsonl").
...
func normalizeBeadsRelPath(relPath string) string {
	...
	// Look for ".beans/" to ensure we match the directory, not a prefix like ".beans-backup"
	if idx := strings.Index(normalized, ".beans/"); idx > 0 {
		// Strip leading path components before .beans
