...
// Hooks are executable scripts in .beans/hooks/ that run after certain events.
package hooks
...
// hooksDir is typically .beans/hooks/ relative to workspace root.
...
func NewRunnerFromWorkspace(workspaceRoot string) *Runner {
	return NewRunner(filepath.Join(workspaceRoot, ".beans", "hooks"))
}
