...
### Important: Don't Include .beans/issues.jsonl Changes

The `.beans/issues.jsonl` file is the project's issue database. **Do not include changes to this file in your PR.** CI will fail if this file is modified.

If you accidentally committed changes to this file, fix it with:

```bash
git checkout origin/main -- .beans/issues.jsonl
git commit --amend
git push --force
```
...
- **Do not include `.beans/issues.jsonl` changes** (see above)
