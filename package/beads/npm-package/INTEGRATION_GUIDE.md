...
5. **Commit .beans/**: The .beans/issues.jsonl file should be committed to git
```
...
5. **bd init** → Imports existing issues from `.beans/issues.jsonl` in git
...
Same process, but:
- Git clone pulls existing `.beans/issues.jsonl`
...
8. Commit changes: Include .beans/issues.jsonl in commits
