...
func TestMyFeature(t *testing.T) {
    tmpDir := t.TempDir()
    testDB := filepath.Join(tmpDir, ".beans", "beads.db")
...
4. **Commit**: Issues auto-sync to `.beans/issues.jsonl` and import after pull
...
**Merge conflicts**: Rare with hash IDs. If conflicts occur, use `git checkout --theirs/.beans/issues.jsonl` and `bd import`. See [docs/GIT_INTEGRATION.md](docs/GIT_INTEGRATION.md).
...
4. **PUSH TO REMOTE - NON-NEGOTIABLE** - This step is MANDATORY. Execute ALL commands below:
   ...
   # If conflicts in .beans/issues.jsonl, resolve thoughtfully:
   #   - git checkout --theirs .beans/issues.jsonl (accept remote)
   #   - bd import -i .beans/issues.jsonl (re-import)
