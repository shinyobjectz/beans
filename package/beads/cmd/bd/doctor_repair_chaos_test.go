...
func TestDoctorRepair_CorruptDatabase_NotADatabase_RebuildFromJSONL(t *testing.T) {
	...
	dbPath := filepath.Join(ws, ".beans", "beads.db")
	jsonlPath := filepath.Join(ws, ".beans", "issues.jsonl")
...
func TestDoctorRepair_CorruptDatabase_NoJSONL_FixFails(t *testing.T) {
	...
	_ = os.Remove(filepath.Join(ws, ".beans", "issues.jsonl"))
	_ = os.Remove(filepath.Join(ws, ".beans", "beads.jsonl"))
	...
	metadata, readErr := os.ReadFile(filepath.Join(ws, ".beans", "metadata.json"))
	if readErr == nil {
...
func TestDoctorRepair_CorruptDatabase_BacksUpSidecars(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(ws, ".beans"))
	if err != nil {
	...
	for _, e := range entries {
		if strings.Contains(e.Name(), ".corrupt.backup.db") {
			backup = filepath.Join(ws, ".beans", e.Name())
			break
		}
	}
	if backup == "" {
		t.Fatalf("expected backup db in .beans, found none")
	}
...
func TestDoctorRepair_CorruptDatabase_ReadOnlyBeadsDir_PermissionsFixMakesWritable(t *testing.T) {
	...
	beadsDir := filepath.Join(ws, ".beans")
	...
	// Make .beans read-only; the Permissions fix should make it writable again.
	...
	if info.Mode().Perm()&0200 == 0 {
		t.Fatalf("expected .beans to be writable after permissions fix, mode=%v", info.Mode().Perm())
	}
}
...
func startDaemonForChaosTest(t *testing.T, bdExe, ws, dbPath string) *exec.Cmd {
	...
	sock := filepath.Join(ws, ".beans", "bd.sock")
...
func runBDWithEnv(ctx context.Context, exe, dir, dbPath string, env map[string]string, args ...string) (string, error) {
	...
	cmd.Env = append(os.Environ(),
		"BEADS_NO_DAEMON=1",
		"BEADS_DIR="+filepath.Join(dir, ".beans"),
	)
	for k, v := range env {
