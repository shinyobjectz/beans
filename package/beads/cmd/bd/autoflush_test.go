...
// target fails.
//
// Bug: When dbPath is set to a relative fallback (e.g., ".beans/beads.db"),
// findJSONLPath() returns ".beans/issues.jsonl" (relative), causing:
//   "Rel: can't make .beans/issues.jsonl relative to /path/to/repo"
//
...
func TestFindJSONLPath_RelativeDbPath(t *testing.T) {
	...
	dbPath = filepath.Join(".beans", beads.CanonicalDatabaseName)

	result := findJSONLPath()

	// Bug: returns ".beans/issues.jsonl" (relative)
...
func TestFindJSONLPath_BEADS_DIR_Relative(t *testing.T) {
	...
	// Create temp directory with .beans structure
	tmpDir := t.TempDir()
	...
	relPath := filepath.Base(tmpDir) + "/.beans"
