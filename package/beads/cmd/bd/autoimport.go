...
// checkGitForIssues checks if git has issues in .beans/beads.jsonl or issues.jsonl
...
func getLocalSyncBranch(beadsDir string) string {
	...
	// Read config.yaml directly from the .beans directory
	configPath := filepath.Join(beadsDir, "config.yaml")
