...
func TestDoctorNoBeadsDir(t *testing.T) {
	...
	// Should fail overall
	if result.OverallOK {
		t.Error("Expected OverallOK to be false when .beans/ directory is missing")
	}
...
func TestDoctorWithBeadsDir(t *testing.T) {
	// Create temporary directory with .beans
	tmpDir := t.TempDir()
...
func TestCheckInstallation(t *testing.T) {
	// Test with missing .beans directory
	tmpDir := t.TempDir()
	...
	// Test with existing .beans directory
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}
...
func TestCheckDatabaseVersionJSONLMode(t *testing.T) {
	// Create temporary directory with .beans but no database
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
...
func TestCheckDatabaseVersionFreshClone(t *testing.T) {
	// Create temporary directory with .beans and JSONL but no database
