...
//   - Pour creates persistent, auditable work in .beans/
...
The resulting mol lives in .beans/ (permanent storage) and is synced with git.
...
func runPour(cmd *cobra.Command, args []string) {
	...
	if dryRun {
		...
		fmt.Printf("Storage: permanent (.beans/)\n\n")
	...
	fmt.Printf("  Phase: liquid (persistent in .beans/)\n")
	if totalAttached > 0 {
