...
var initCmd = &cobra.Command{
	...
	Long: `Initialize bd in the current directory by creating a .beans/ directory
and database file. Optionally specify a custom issue prefix.

With --no-db: creates .beans/ directory and issues.jsonl file instead of SQLite database.

With --from-jsonl: imports from the current .beans/issues.jsonl file on disk instead
...
With --stealth: configures per-repository git settings for invisible beads usage:
  ...
  Perfect for personal use without affecting repo collaborators.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		// otherwise default to `.beans/beads.db` for SQLite.
		// If there's a redirect file, use the redirect target (GH#bd-0qel)
		initDBPath := dbPath
		if backend == configfile.BackendDolt {
			initDBPath = filepath.Join(".beans", "dolt")
		} else if initDBPath == "" {
			// Check for redirect in local .beans
			localBeadsDir := filepath.Join(".", ".beans")
		...
		// Determine if we should create .beans/ directory in CWD or main repo root
		// For worktrees, .beans should always be in the main repository root
		cwd, err := os.Getwd()
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: failed to get current directory: %v\n", err)
			os.Exit(1)
		...
		if isWorktree {
			...
			fmt.Fprintf(os.Stderr, "Git worktrees share the .beans database from the main repository.\n")
		...
		localBeadsDir := filepath.Join(cwd, ".beans")
		beadsDir = beads.FollowRedirect(localBeadsDir)

		// Prevent nested .beans directories
		// Check if current working directory is inside a .beans directory
		if strings.Contains(filepath.Clean(cwd), string(filepath.Separator)+".beans"+string(filepath.Separator)) ||
			strings.HasSuffix(filepath.Clean(cwd), string(filepath.Separator)+".beans") {
			fmt.Fprintf(os.Stderr, "Error: cannot initialize bd inside a .beans directory\n")
			fmt.Fprintf(os.Stderr, "Current directory: %s\n", cwd)
			fmt.Fprintf(os.Stderr, "Please run 'bd init' from outside the .beans directory.\n")
			os.Exit(1)
		}
		...
		if useLocalBeads {
			...
			if err := os.MkdirAll(beadsDir, 0750); err != nil {
				fmt.Fprintf(os.Stderr, "Error: failed to create .beans directory: %v\n", err)
				os.Exit(1)
			}
			...
			// Create/update .gitignore in .beans directory (idempotent - always update to latest)
		...
		// For SQLite: parent of .beans/beads.db. For Dolt: parent of .beans/dolt.
		...
		//   `.beans/issues.jsonl` on first open (factory_dolt.go) when present.
		if backend == configfile.BackendSQLite {
		...
		// For Dolt backend, install hooks to .beans/hooks/ (uses git config core.hooksPath)
...
func init() {
	...
	initCmd.Flags().Bool("from-jsonl", false, "Import from current .beans/issues.jsonl file instead of git history (preserves manual cleanups)")
...
func migrateOldDatabases(targetPath string, quiet bool) error {
	...
	if err := os.MkdirAll(targetDir, 0750); err != nil {
		return fmt.Errorf("failed to create .beans directory: %w", err)
	}

	// Look for existing .db files in the .beans directory
...
func checkExistingBeadsData(prefix string) error {
	...
	// Determine where to check for .beans directory
	// Guard with isGitRepo() check first - on Windows, git commands may hang
	// when run outside a git repository (GH#727)
	var beadsDir string
	if isGitRepo() && git.IsWorktree() {
		// For worktrees, .beans should be in the main repository root
		mainRepoRoot, err := git.GetMainRepoRoot()
		if err != nil {
		...
		beadsDir = filepath.Join(cwd, ".beans")
	}

	// Check if .beans directory exists
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		return nil // No .beans directory, safe to init
	}

	// Check for existing database (SQLite or Dolt)
	//
	// NOTE: For Dolt backend, the "database" is a directory at `.beans/dolt/`.
...
To completely reinitialize (data loss warning):
  rm -rf .beans && bd init --backend dolt --prefix %s
