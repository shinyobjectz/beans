...
Subcommands:
  ...
  tombstones  Convert deletions.jsonl to inline tombstones`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		"message": "No .beans directory found. Run 'bd init' first.",
		})
		os.Exit(1)
		} else {
		FatalErrorWithHint("no .beans directory found", "run 'bd init' to initialize bd")
		}
		}
	...
	// Detect all database files
		...
		if len(databases) == 0 {
			if jsonOutput {
				outputJSON(map[string]interface{}{
					"status":  "no_databases",
					"message": "No database files found in .beans/",
				})
			} else {
...
func handleInspect() {
	...
	if beadsDir == "" {
		if jsonOutput {
			...
			fmt.Fprintf(os.Stderr, "Error: no .beans directory found\n")
			fmt.Fprintf(os.Stderr, "Hint: run 'bd init' to initialize bd\n")
		}
		os.Exit(1)
	}
