...
//
// The redirect file contains a relative path from the worktree's .beans directory
// to the main repository's .beans directory. The depth of ../ components depends
// on how deeply nested the worktree is.
func TestWorktreeRedirectDepth(t *testing.T) {
	// Create a temporary repo structure
	tmpDir := t.TempDir()

	// Main repo's .beans directory
	mainBeadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(mainBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create main .beans dir: %v", err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create worktree .beans directory
			...
			// Verify the relative path ends with .beans
			if !strings.HasSuffix(relPath, ".beans") {
				t.Errorf("expected relPath to end with .beans, got %q", relPath)
			}
...
func TestWorktreeRedirectWithRelativeMainBeadsDir(t *testing.T) {
	...
	// Compute relative path from worktree's .beans to main .beans
	relPath, err := filepath.Rel(worktreeBeadsDir, absMainBeadsDir)
	if err != nil {
		t.Fatalf("filepath.Rel() failed: %v", err)
	}

	// Verify the path looks correct (should be ../../.beans)
...
func TestWorktreeRedirectWithoutFix(t *testing.T) {
	...
	// The buggy relPath will be something like "../../../.beans" when it should be "../../.beans"
