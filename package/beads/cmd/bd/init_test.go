...
func TestInitCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify .beans directory was created
			beadsDir := filepath.Join(tmpDir, ".beans")
			if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
				t.Error(".beans directory was not created")
			}
...
func TestInitWithSyncBranch(t *testing.T) {
	...
	dbFilePath := filepath.Join(tmpDir, ".beans", "beads.db")
...
// This fixes the issue where fresh clones show .beans/issues.jsonl as modified.
func TestInitWithSyncBranchSetsGitExclude(t *testing.T) {
	...
	if !strings.Contains(excludeContent, ".beans/interactions.jsonl") {
		t.Errorf("Expected .git/info/exclude to contain '.beans/interactions.jsonl', got:\n%s", excludeContent)
	}
}
...
func TestInitWithExistingSyncBranchConfig(t *testing.T) {
	...
	// Create .beans directory with config.yaml containing sync-branch (simulating a clone)
	beadsDir := filepath.Join(tmpDir, ".beans")
...
`
	...
	excludeContent := string(content)
	if !strings.Contains(excludeContent, ".beans/interactions.jsonl") {
		t.Errorf("Expected .git/info/exclude to contain '.beans/interactions.jsonl' when sync-branch is in config.yaml, got:\n%s", excludeContent)
	}
}
...
func TestInitAlreadyInitialized(t *testing.T) {
	...
	dbPath := filepath.Join(tmpDir, ".beans", "beads.db")
	store, err := openExistingTestDB(t, dbPath)
	if err != nil {
		t.Fatalf("Failed to open database: %v", err)
	}
...
func TestInitWithCustomDBPath(t *testing.T) {
	...
	t.Run("init with BEADS_DB pointing to custom path", func(t *testing.T) {
		...
		// Verify .beans/ directory was NOT created in work directory
		if _, err := os.Stat(filepath.Join(workDir, ".beans")); err == nil {
			t.Error(".beans/ directory should not be created when using BEADS_DB env var")
		}
	})
	...
	// Test that BEADS_DB path containing ".beans" doesn't create CWD/.beans
	t.Run("init with BEADS_DB path containing .beans", func(t *testing.T) {
		dbPath = "" // Reset global
		// Path contains ".beans" but is outside work directory
		customPath := filepath.Join(tmpDir, "storage", ".beans-backup", "test.db")
		...
		if err := rootCmd.Execute(); err != nil {
			t.Fatalf("Init with custom .beans path failed: %v", err)
		}
		...
		// Verify .beans/ directory was NOT created in work directory
		if _, err := os.Stat(filepath.Join(workDir, ".beans")); err == nil {
			t.Error(".beans/ directory should not be created in CWD when BEADS_DB path contains .beans")
		}
	})
	...
	t.Run("BEADS_DB with subdirectories", func(t *testing.T) {
		...
		// Verify .beans/ directory was NOT created in work directory
		if _, err := os.Stat(filepath.Join(workDir, ".beans")); err == nil {
			t.Error(".beans/ directory should not be created in CWD when BEADS_DB is set")
		}
	})
}
...
func TestInitNoDbMode(t *testing.T) {
	...
	// Set BEADS_DIR to prevent git repo detection from finding project's .beans
	origBeadsDir := os.Getenv("BEADS_DIR")
	os.Setenv("BEADS_DIR", filepath.Join(tmpDir, ".beans"))
...
func TestInitMergeDriverAutoConfiguration(t *testing.T) {
	t.Run("merge driver auto-configured during init", func(t *testing.T) {
		...
		output, err := runCommandInDirWithOutput(tmpDir, "git", "config", "merge.beans.driver")
		if err != nil {
		...
		if !strings.Contains(string(content), ".beans/issues.jsonl merge=beads") {
	...
	t.Run("skip merge driver with flag", func(t *testing.T) {
		...
		_, err := runCommandInDirWithOutput(tmpDir, "git", "config", "--local", "merge.beans.driver")
		if err == nil {
	...
	t.Run("non-git repo skips merge driver silently", func(t *testing.T) {
		...
		// Verify .beans was still created
		beadsDir := filepath.Join(tmpDir, ".beans")
		if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
			t.Error(".beans directory should be created even without git")
		}
	})
	...
	t.Run("detect already-installed merge driver", func(t *testing.T) {
		...
		if err := runCommandInDir(tmpDir, "git", "config", "merge.beans.driver", "bd merge %A %O %A %B"); err != nil {
			t.Fatalf("Failed to set git config: %v", err)
		}

		// Create .gitattributes with merge driver
		gitattrsPath := filepath.Join(tmpDir, ".gitattributes")
		initialContent := "# Existing config\n.beans/issues.jsonl merge=beads\n"
		...
		count := strings.Count(contentStr, ".beans/issues.jsonl merge=beads")
		if count != 1 {
	...
	t.Run("append to existing .gitattributes", func(t *testing.T) {
		...
		if !strings.Contains(contentStr, ".beans/issues.jsonl merge=beads") {
		...
		beadsIdx := strings.Index(contentStr, ".beans/issues.jsonl")
		if txtIdx >= beadsIdx {
	...
	t.Run("verify git config has correct settings", func(t *testing.T) {
		...
		// Verify merge.beans.driver is set correctly
		driver, err := runCommandInDirWithOutput(tmpDir, "git", "config", "merge.beans.driver")
		if err != nil {
			t.Fatalf("Failed to get merge.beans.driver: %v", err)
		}
		driver = strings.TrimSpace(driver)
		expected := "bd merge %A %O %A %B"
		if driver != expected {
			t.Errorf("Expected merge.beans.driver to be %q, got %q", expected, driver)
		}

		// Verify merge.beans.name is set
		name, err := runCommandInDirWithOutput(tmpDir, "git", "config", "merge.beans.name")
		if err != nil {
			t.Fatalf("Failed to get merge.beans.name: %v", err)
		}
		name = strings.TrimSpace(name)
		if !strings.Contains(name, "bd") {
			t.Errorf("Expected merge.beans.name to contain 'bd', got %q", name)
		}
	})
	...
	t.Run("auto-repair stale merge driver with invalid placeholders", func(t *testing.T) {
		...
		if err := runCommandInDir(tmpDir, "git", "config", "merge.beans.driver", "bd merge %L %R"); err != nil {
			t.Fatalf("Failed to set stale git config: %v", err)
		}

		// Create .gitattributes with merge driver
		gitattrsPath := filepath.Join(tmpDir, ".gitattributes")
		if err := os.WriteFile(gitattrsPath, []byte(".beans/beads.jsonl merge=beads\n"), 0644); err != nil {
			t.Fatalf("Failed to create .gitattributes: %v", err)
		}
	...
	t.Run("detect canonical issues.jsonl filename in gitattributes", func(t *testing.T) {
		...
		gitattrsPath := filepath.Join(tmpDir, ".gitattributes")
		if err := os.WriteFile(gitattrsPath, []byte(".beans/issues.jsonl merge=beads\n"), 0644); err != nil {
			t.Fatalf("Failed to create .gitattributes: %v", err)
		}
...
func TestSetupGlobalGitIgnore_ReadOnly(t *testing.T) {
	t.Run("read-only file", func(t *testing.T) {
		...
		if !strings.Contains(output, "/test/project/.beans/") {
			t.Error("expected .beans pattern in output")
		}
	})
...
// not in the local .beans directory. (GH#bd-0qel)
func TestInitWithRedirect(t *testing.T) {
	...
	// Create local .beans with redirect file pointing to target
	localBeadsDir := filepath.Join(projectDir, ".beans")
	if err := os.MkdirAll(localBeadsDir, 0755); err != nil {
		t.Fatal(err)
	}

	// Create target .beans directory (the redirect destination)
	targetBeadsDir := filepath.Join(tmpDir, "canonical", ".beans")
	...
	// Relative path from project/.beans to canonical/.beans is ../canonical/.beans
	if err := os.WriteFile(redirectPath, []byte("../canonical/.beans\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(localDBPath); err == nil {
		t.Errorf("Database was incorrectly created in local .beans: %s (should be in redirect target)", localDBPath)
	}
...
func TestInitWithRedirectToExistingDatabase(t *testing.T) {
	...
	// Create canonical .beans directory with EXISTING database
	canonicalDir := filepath.Join(tmpDir, "canonical")
	canonicalBeadsDir := filepath.Join(canonicalDir, ".beans")
	...
	projectDir := filepath.Join(tmpDir, "project")
	projectBeadsDir := filepath.Join(projectDir, ".beans")
