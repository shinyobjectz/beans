...
func failIfProductionDatabase(t *testing.T, dbPath string) {
	...
	// Check if database is in .beans/ directory of this git repository
	beadsPath := ""
	...
	// The .beans directory should be at the root of the git repository
	// For worktrees, gitDir points to the main repo's .git directory
	repoRoot := filepath.Dir(gitDirAbs)
	beadsPath = filepath.Join(repoRoot, ".beans")
	
	if strings.HasPrefix(absPath, beadsPath) {
		// Database is in .beans/ directory of a git repository
