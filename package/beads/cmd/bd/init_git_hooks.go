...
# .beans/issues.jsonl before the commit is created, preventing the
...
# For worktrees, .beans is in the main repository root, not the worktree
BEADS_DIR=""
if git rev-parse --git-dir >/dev/null 2>&1; then
    # Check if we're in a worktree
    if [ "$(git rev-parse --git-dir)" != "$(git rev-parse --git-common-dir)" ]; then
        # Worktree: .beans is in main repo root
        MAIN_REPO_ROOT="$(git rev-parse --git-common-dir)"
        MAIN_REPO_ROOT="$(dirname "$MAIN_REPO_ROOT")"
        if [ -d "$MAIN_REPO_ROOT/.beans" ]; then
            BEADS_DIR="$MAIN_REPO_ROOT/.beans"
        fi
    else
...
if [ -f "$BEADS_DIR/issues.jsonl" ]; then
    ...
    # For worktrees: .beans is in the main repo's working tree, not this worktree
...
#
# This hook imports updated issues from .beans/issues.jsonl after a
...
// Note: This runs during bd init BEFORE .beans exists, so it runs git in CWD.
func mergeDriverInstalled() bool {
	...
	hasCanonical := strings.Contains(string(content), ".beans/issues.jsonl") &&
		strings.Contains(string(content), "merge=beads")
	hasLegacy := strings.Contains(string(content), ".beans/beads.jsonl") &&
		strings.Contains(string(content), "merge=beads")
...
#
# This hook ensures that any pending bd issue changes are flushed to
# .beans/issues.jsonl before the commit.
#
...
func installMergeDriver() error {
	...
	cmd := exec.Command("git", "config", "merge.beans.driver", "bd merge %A %O %A %B")
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to configure git merge driver: %w\n%s", err, output)
	}

	cmd = exec.Command("git", "config", "merge.beans.name", "bd JSONL merge driver")
	if output, err := cmd.CombinedOutput(); err != nil {
	...
	hasBeadsMerge := (strings.Contains(existingContent, ".beans/issues.jsonl") ||
		strings.Contains(existingContent, ".beans/beads.jsonl")) &&
