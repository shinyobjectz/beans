...
func testFreshCloneAutoImport(t *testing.T) {
	...
	// Create .beans directory with issues.jsonl (canonical name)
	...
	runCmd(t, dir, "git", "add", ".beans/issues.jsonl")
	...
	expectedPath := normalizeGitPath(".beans/issues.jsonl")
...
func testDatabaseRemovalScenario(t *testing.T) {
	...
	// Simulate rm -rf .beans/ followed by partial bd init
...
func testLegacyFilenameSupport(t *testing.T) {
	...
	// Create .beans directory with issues.jsonl (legacy)
	beadsDir := filepath.Join(dir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatalf("Failed to create .beans dir: %v", err)
	}
...
func testPrecedenceTest(t *testing.T) {
	...
	// Create .beans directory with BOTH files
	beadsDir := filepath.Join(dir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatalf("Failed to create .beans dir: %v", err)
	}
