...
func init() {
	...
	daemonCmd.Flags().String("log", "", "Log file path (default: .beans/daemon.log)")
...
func runDaemonLoop(interval time.Duration, autoCommit, autoPush, autoPull, localMode bool, logPath, pidFile, logLevel string, logJSON, federation bool, federationPort, remotesapiPort int) {
	...
	// Dolt uses a directory-backed store (typically .beans/dolt), so the "beads.db"
	...
	// Get workspace path (.beans directory) - beadsDir already defined above
	// Get actual workspace root (parent of .beans)
