...
The file defaults to .beans/beads.jsonl if not specified.

Examples:
  bd resolve-conflicts                    # Resolve conflicts in .beans/beads.jsonl
...
func init() {
	...
	resolveConflictsCmd.Flags().StringVar(&resolveConflictsPath, "path", ".", "Path to repository with .beans directory")
...
func runResolveConflicts(cmd *cobra.Command, args []string) {
	...
	if len(args) > 0 {
		...
		filePath = filepath.Join(resolveConflictsPath, ".beans", "beads.jsonl")
	}
