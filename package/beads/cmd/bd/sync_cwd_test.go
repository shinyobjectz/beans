...
//
// The bug (oss-lbp): Running from .beans/ caused paths like "oss/" to become
// ".beans/oss/" instead of "{repo}/oss/". This test ensures the fix works
...
func TestMultiRepoPathResolutionCWDInvariant(t *testing.T) {
	...
	// Create .beans directory structure
	...
	ossBeadsDir := filepath.Join(ossDir, ".beans")
	if err := os.MkdirAll(ossBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create oss/.beans dir: %v", err)
	}
...
`
	...
	// T041: Test from .beans/ directory (the bug trigger location)
	t.Run("T041_from_beads_directory", func(t *testing.T) {
		if err := os.Chdir(beadsDir); err != nil {
			t.Fatalf("chdir to .beans failed: %v", err)
		}
		git.ResetCaches()
		...
		t.Logf("From .beans/: additional[0] = %q", multiRepo.Additional[0])
		t.Logf("ConfigFileUsed() = %q", configUsed)
	})
...
func TestExportToMultiRepoCWDInvariant(t *testing.T) {
	...
	// Create oss/.beans directory
	ossBeadsDir := filepath.Join(tmpDir, "oss", ".beans")
	if err := os.MkdirAll(ossBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create oss/.beans dir: %v", err)
	}
...
`
	...
	runExportTest := func(t *testing.T, testCwd string) string {
		...
		// The export should create issues.jsonl in oss/.beans/
	...
	// Test from .beans/ directory
	t.Run("export_from_beads_dir", func(t *testing.T) {
		path := runExportTest(t, beadsDir)
		t.Logf("Export from .beans/ created: %s", path)

		// Key assertion: should NOT create .beans/oss/.beans/issues.jsonl
		badPath := filepath.Join(beadsDir, "oss", ".beans", "issues.jsonl")
...
`
		...
		// Simulate daemon context: CWD is .beans/
		if err := os.Chdir(beadsDir); err != nil {
			store.Close()
			t.Fatalf("chdir to .beans/ failed: %v", err)
		}
		...
		expectedPath := filepath.Join(ossBeadsDir, "issues.jsonl")
		if _, err := os.Stat(expectedPath); os.IsNotExist(err) {
			t.Errorf("expected %s to exist (sync-branch mode from .beans/)", expectedPath)
		}
	...
	t.Run("T052_external_beads_dir_mode", func(t *testing.T) {
		...
		// Create .beans in external repo
		externalBeadsDir := filepath.Join(externalDir, ".beans")
		if err := os.MkdirAll(externalBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create external .beans: %v", err)
		}

		// Create oss/.beans in external repo (sibling to external .beans)
		ossBeadsDir := filepath.Join(externalDir, "oss", ".beans")
		if err := os.MkdirAll(ossBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create oss/.beans: %v", err)
		}
