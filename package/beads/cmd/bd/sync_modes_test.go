...
func TestExternalBeadsDirWithPullFirst(t *testing.T) {
	...
	// Create .beans directory in external repo
	...
	_ = exec.Command("git", "-C", externalDir, "add", ".beans").Run()
...
// for multi-machine E2E testing. Each clone has its own .beans directory for isolation.
//
// Returns:
...
// withBeadsDir runs a function with BEADS_DIR set to the specified directory's .beans subdirectory.
...
func TestSyncBranchE2E(t *testing.T) {
	...
	// Machine A: Create .beans directory and bd-1
	beadsDirA := filepath.Join(machineA, ".beans")
	if err := os.MkdirAll(beadsDirA, 0755); err != nil {
		t.Fatalf("failed to create .beans dir for A: %v", err)
	}
	...
	// Machine B: Create .beans directory and bd-2
	beadsDirB := filepath.Join(machineB, ".beans")
	if err := os.MkdirAll(beadsDirB, 0755); err != nil {
		t.Fatalf("failed to create .beans dir for B: %v", err)
	}
...
// Worse, some code paths could commit all staged files instead of just .beans/.
