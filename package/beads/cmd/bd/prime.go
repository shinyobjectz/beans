...
- Place a .beans/PRIME.md file to override the default output entirely.
- Use --export to dump the default content for customization.`,
	Run: func(cmd *cobra.Command, args []string) {
		// Find .beans/ directory (supports both database and JSONL-only mode)
		beadsDir := beads.FindBeadsDir()
		if beadsDir == "" {
		...
		// Check local .beans/ first (even if redirected), then redirected location
		if !primeExportMode {
			localPrimePath := filepath.Join(".beans", "PRIME.md")
...
func getRedirectNotice(verbose bool) string {
	...
	if verbose {
		return fmt.Sprintf(`> ⚠️ **Redirected**: Local .beans → %s
...
> Hooks auto-call this in Claude Code when .beans/ detected
