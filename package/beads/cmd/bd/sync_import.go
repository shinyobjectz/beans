...
// This avoids path resolution issues when running from directories with .beans/redirect.
...
func doSyncFromMain(ctx context.Context, jsonlPath string, renameOnImport bool, dryRun bool, noGitHistory bool) error {
	...
	if dryRun {
		...
		fmt.Printf("  2. Checkout .beans/ from %s/main\n", remote)
	...
	// Step 2: Checkout .beans/ directory from main
	fmt.Printf("→ Checking out beads from %s/%s...\n", remote, defaultBranch)
	checkoutCmd := rc.GitCmd(ctx, "checkout", fmt.Sprintf("%s/%s", remote, defaultBranch), "--", ".beans/")
	if output, err := checkoutCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("git checkout .beans/ from %s/%s failed: %w\n%s", remote, defaultBranch, err, output)
	}
