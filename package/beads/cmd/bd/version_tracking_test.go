...
func TestTrackBdVersion_NoBeadsDir(t *testing.T) {
	...
	// Change to temp directory with no .beans
	tmpDir := t.TempDir()
	t.Chdir(tmpDir)
	...
	// or finding the worktree's main repository .beans directory
	t.Setenv("BEADS_DIR", tmpDir)

	// trackBdVersion should silently succeed
	trackBdVersion()

	// Should not detect upgrade when no .beans dir exists
	if versionUpgradeDetected {
		t.Error("Expected no upgrade detection when .beans directory doesn't exist")
	}
}
...
func TestTrackBdVersion_FirstRun(t *testing.T) {
	...
	// Create temp .beans directory with a project file (bd-420)
	...
	// This prevents finding the actual .beans in a git worktree
