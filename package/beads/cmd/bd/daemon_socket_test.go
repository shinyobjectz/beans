...
func TestSocketPathDefaultBehavior(t *testing.T) {
	...
	pidFile := "/path/to/.beans/daemon.pid"
	got := getSocketPathForPID(pidFile)
	want := "/path/to/.beans/bd.sock"
	if got != want {
...
func TestSocketPathForPIDLongPath(t *testing.T) {
	...
	// Create a path that would exceed the 103-byte limit when .beans/bd.sock is appended
	// /long/path/.beans/daemon.pid -> workspace is /long/path
	// socket would be /long/path/.beans/bd.sock
	longWorkspace := "/" + strings.Repeat("a", 90) // 91 bytes
	pidFile := filepath.Join(longWorkspace, ".beans", "daemon.pid")

	got := getSocketPathForPID(pidFile)

	// Should NOT be the natural path (which would be too long)
	naturalPath := filepath.Join(longWorkspace, ".beans", "bd.sock")
	if got == naturalPath {
...
func TestSocketPathForPIDClientDaemonAgreement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pidFile := filepath.Join(tt.workspacePath, ".beans", "daemon.pid")
