...
// BEADS_JSONL env var first, then using .beans/issues.jsonl next to the database).
//
...
//
// Creates the .beans directory if it doesn't exist (important for new databases).
...
func findJSONLPath() string {
	...
	// In --no-db mode, dbPath may be empty. Fall back to locating the .beans directory.
	if jsonlPath == "" {
...
func recordFlushFailure(err error) {
	...
	if failCount >= 3 {
		...
		fmt.Fprintf(os.Stderr, "%s\n\n", ui.RenderFail("⚠️  Run 'bd export -o .beans/issues.jsonl' manually to fix."))
	}
}
