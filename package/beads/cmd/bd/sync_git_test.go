...
// setupGitRepoWithBeads creates a temporary git repository with a .beans directory.
...
func setupGitRepoWithBeads(t *testing.T) (repoPath string, cleanup func()) {
	...
	// Create .beans directory with issues.jsonl
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		_ = os.Chdir(originalWd)
		t.Fatalf("failed to create .beans directory: %v", err)
	}
	...
	_ = exec.Command("git", "add", ".beans").Run()
...
// setupRedirectedBeadsRepo creates two git repos: source with redirect, target with actual .beans.
...
func setupRedirectedBeadsRepo(t *testing.T) (sourcePath, targetPath string, cleanup func()) {
	...
	// Create target repo with actual .beans
	targetPath = filepath.Join(baseDir, "target")
	if err := os.MkdirAll(targetPath, 0755); err != nil {
		t.Fatalf("failed to create target directory: %v", err)
	}

	targetBeadsDir := filepath.Join(targetPath, ".beans")
	if err := os.MkdirAll(targetBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create target .beans directory: %v", err)
	}
	...
	// Commit in target
	cmd = exec.Command("git", "add", ".beans")
	cmd.Dir = targetPath
	_ = cmd.Run()
	...
	sourceBeadsDir := filepath.Join(sourcePath, ".beans")
	if err := os.MkdirAll(sourceBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create source .beans directory: %v", err)
	}

	// Write redirect file pointing to target
	redirectPath := filepath.Join(sourceBeadsDir, "redirect")
	// Use relative path: ../target/.beans
	if err := os.WriteFile(redirectPath, []byte("../target/.beans\n"), 0644); err != nil {
...
func TestGitHasBeadsChanges_WithChanges(t *testing.T) {
	...
	jsonlPath := filepath.Join(repoPath, ".beans", "issues.jsonl")
...
func TestGitHasBeadsChanges_WithRedirect_NoChanges(t *testing.T) {
	...
	// Set BEADS_DIR to point to source's .beans (which has the redirect)
	oldBeadsDir := os.Getenv("BEADS_DIR")
	os.Setenv("BEADS_DIR", filepath.Join(sourcePath, ".beans"))
...
func TestGitHasBeadsChanges_WithRedirect_WithChanges(t *testing.T) {
	...
	jsonlPath := filepath.Join(targetPath, ".beans", "issues.jsonl")
	if err := os.WriteFile(jsonlPath, []byte(`{"id":"test-2"}`+"\n"), 0644); err != nil {
		t.Fatalf("failed to modify issues.jsonl: %v", err)
	}
...
func TestParseGitStatusForBeadsChanges(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "staged added",
			status:   "A  .beans/issues.jsonl",
			expected: true,
		},
		{
			name:     "added then modified",
			status:   "AM .beans/issues.jsonl",
			expected: true,
		},
		...
		{
			name:     "renamed file",
			status:   "R  old.jsonl -> .beans/issues.jsonl",
			expected: false,
		},
		{
			name:     "copied file",
			status:   "C  source.jsonl -> .beans/issues.jsonl",
			expected: false,
		},
		{
...
func TestGitBranchHasUpstream(t *testing.T) {
	...
	// Create .beans directory (required for RepoContext)
	beadsDir := filepath.Join(localDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
...
// It verifies that config.yaml in .beans/ is not overwritten during sync operations.
// The bug was caused by restoreBeadsDirFromBranch() which ran:
//   git checkout HEAD -- .beans/
// This restored the ENTIRE .beans/ directory, including user's uncommitted config.yaml.
...
func TestConfigPreservedDuringSync(t *testing.T) {
	...
	t.Run("uncommitted config.yaml not overwritten by git operations", func(t *testing.T) {
		// Setup: Create git repo with .beans directory
		repoPath, cleanup := setupGitRepoWithBeads(t)
		defer cleanup()

		beadsDir := filepath.Join(repoPath, ".beans")
		...
		// Commit the config
		_ = exec.Command("git", "add", ".beans/config.yaml").Run()
		...
		// Simulate what the old buggy code did: git checkout HEAD -- .beans/
		...
		cmd := exec.Command("git", "status", "--porcelain", ".beans/config.yaml")
	...
	t.Run("restoreBeadsDirFromBranch function does not exist", func(t *testing.T) {
		...
		if strings.Contains(string(content), "func restoreBeadsDirFromBranch") {
			t.Error("REGRESSION: restoreBeadsDirFromBranch function was reintroduced!\n" +
				"This function caused GH#1100 by restoring entire .beans/ directory.\n" +
