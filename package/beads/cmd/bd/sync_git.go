...
// gitHasBeadsChanges checks if any tracked files in .beans/ have uncommitted changes.
// Uses RepoContext to ensure git commands run in the correct repository.
...
func gitHasBeadsChanges(ctx context.Context) (bool, error) {
	...
	// rc.GitCmd runs in rc.RepoRoot which is the repo containing .beans/
...
// gitCommitBeadsDir stages and commits only sync-related files in .beans/
...
func gitCommitBeadsDir(ctx context.Context, message string) error {
	...
	// Commit only .beans/ files using -- pathspec
...
func hasJSONLConflict() bool {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if status == "UU" || status == "AA" || status == "DD" ||
			...
			// Check for beads JSONL files (issues.jsonl or beads.jsonl in .beans/)
...
func checkMergeDriverConfig() {
	...
	cmd := rc.GitCmd(ctx, "config", "merge.beans.driver")
	output, err := cmd.Output()
	if err != nil {
	...
	if strings.Contains(currentConfig, "%L") || strings.Contains(currentConfig, "%R") {
		...
		fmt.Fprintf(os.Stderr, "   Or manually: git config merge.beans.driver \"bd merge %%A %%O %%A %%B\"\n\n")
	}
}

// gitHasUncommittedBeadsChanges checks if .beans/issues.jsonl has uncommitted changes.
