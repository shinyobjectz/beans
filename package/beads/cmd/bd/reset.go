...
This command removes:
  - The .beans directory (database, JSONL, config)
...
func collectResetItems(gitCommonDir, beadsDir string) []resetItem {
	...
	items = append(items, resetItem{
		Type:        "directory",
		Path:        beadsDir,
		Description: "Remove .beans directory (database, JSONL, config)",
	})
...
func hasMergeDriverConfig() bool {
	cmd := exec.Command("git", "config", "--get", "merge.beans.driver")
...
func performReset(items []resetItem, _, beadsDir string) {
	...
	for _, item := range items {
		...
		case "config":
			...
			_ = exec.Command("git", "config", "--unset", "merge.beans.driver").Run()
			_ = exec.Command("git", "config", "--unset", "merge.beans.name").Run()
			if !jsonOutput {
		...
		case "directory":
			if err := os.RemoveAll(item.Path); err != nil {
				errors = append(errors, fmt.Sprintf("failed to remove .beans: %v", err))
			} else if !jsonOutput {
				fmt.Printf("%s Removed .beans directory\n", ui.RenderPass("✓"))
			}
		}
	}
