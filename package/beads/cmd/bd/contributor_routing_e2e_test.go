...
func TestContributorRoutingTracer(t *testing.T) {
	...
	t.Run("DetermineTargetRepo_contributor_routes_to_planning", func(t *testing.T) {
		config := &routing.RoutingConfig{
			Mode:            "auto",
			ContributorRepo: "~/.beans-planning",
		}

		got := routing.DetermineTargetRepo(config, routing.Contributor, ".")
		if got != "~/.beans-planning" {
			t.Errorf("DetermineTargetRepo() = %q, want %q", got, "~/.beans-planning")
		}
	})
	...
	t.Run("E2E_routing_decision_with_store", func(t *testing.T) {
		...
		// Create project .beans directory
		projectBeadsDir := filepath.Join(projectDir, ".beans")
		if err := os.MkdirAll(projectBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create project .beans dir: %v", err)
		}

		// Create planning .beans directory
		planningBeadsDir := filepath.Join(planningDir, ".beans")
		if err := os.MkdirAll(planningBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create planning .beans dir: %v", err)
		}
...
func (env *contributorRoutingEnv) initProjectStore(syncMode string) *sqlite.SQLiteStorage {
	env.t.Helper()
	projectDBPath := filepath.Join(env.projectDir, ".beans", "beads.db")
...
func (env *contributorRoutingEnv) initPlanningStore() *sqlite.SQLiteStorage {
	env.t.Helper()
	planningDBPath := filepath.Join(env.planningDir, ".beans", "beads.db")
...
func TestExplicitRepoOverride(t *testing.T) {
	...
	overrideBeadsDir := filepath.Join(overrideDir, ".beans")
	if err := os.MkdirAll(overrideBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create override .beans dir: %v", err)
	}
...
func TestBEADS_DIRPrecedence(t *testing.T) {
	...
	if err := os.MkdirAll(externalBeadsDir, 0755); err != nil {
		t.Fatalf("failed to create external .beans dir: %v", err)
	}
	...
	// 4. Current directory's .beans/ (default)
