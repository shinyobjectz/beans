...
// ensureBeadsDir ensures the local beads directory exists (.beans in the current workspace)
func ensureBeadsDir() (string, error) {
	...
	if dbPath != "" {
		...
		if foundDB := beads.FindDatabasePath(); foundDB != "" {
			...
			// No database found - error out instead of falling back to ~/.beans
...
func getSocketPathForPID(pidFile string) string {
	...
	// PID file is in .beans/, so workspace is parent of that
