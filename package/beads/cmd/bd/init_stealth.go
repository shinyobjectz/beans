...
// - Patterns are relative to repo root, so ".beans/" works correctly
...
func setupForkExclude(verbose bool) error {
	...
	patterns := []string{".beans/", "**/RECOVERY*.md", "**/SESSION*.md"}
	var toAdd []string
	for _, p := range patterns {
		// Check for exact line match (pattern alone on a line)
		// This avoids false positives like ".beans/issues.jsonl" matching ".beans/"
...
func setupGlobalGitIgnore(homeDir string, projectPath string, verbose bool) error {
	...
	beadsPattern := projectPath + "/.beans/"
