...
type WhereResult struct {
	Path           string `json:"path"`                      // Active .beans directory path
...
which .beans directory is actually being used.
...
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Check if we got here via redirect by looking for the original .beans directory
		// Walk up from cwd to find any .beans with a redirect file
...
// findOriginalBeadsDir walks up from cwd looking for a .beans directory with a redirect file
// Returns the original .beans path if found, empty string otherwise
func findOriginalBeadsDir() string {
	...
	// Walk up directory tree looking for .beans with redirect
	for dir := cwd; dir != "/" && dir != "."; {
		...
		if info, err := os.Stat(beadsDir); err == nil && info.IsDir() {
			...
			// Found .beans without redirect - this is the actual location
			return ""
		}
