...
func TestShouldDisableDaemonForWorktree(t *testing.T) {
	...
	t.Run("returns true in worktree without sync-branch", func(t *testing.T) {
		...
		// Set BEADS_DIR to the test's .beans directory to prevent
		// git repo detection from finding the project's .beans
		origBeadsDir := os.Getenv("BEADS_DIR")
		os.Setenv("BEADS_DIR", mainDir+"/.beans")
		defer func() {
			if origBeadsDir != "" {
...
func setupWorktreeTestRepo(t *testing.T) (mainDir, worktreeDir string) {
	...
	// Create .beans directory with empty config (no sync-branch)
	beadsDir := mainDir + "/.beans"
