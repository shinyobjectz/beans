...
Configuration is stored per-project in .beans/*.db and is version-control-friendly.
...
Examples:
  ...
  bd config validate --json`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Find repo root by walking up to find .beans directory
		repoPath := findBeadsRepoRoot(cwd)
		if repoPath == "" {
			fmt.Fprintf(os.Stderr, "Error: not in a beads repository (no .beans directory found)\n")
			os.Exit(1)
		}
...
func validateSyncConfig(repoPath string) []string {
	...
	configPath := filepath.Join(repoPath, ".beans", "config.yaml")
	v := viper.New()
...
// findBeadsRepoRoot walks up from the given path to find the repo root (containing .beans)
