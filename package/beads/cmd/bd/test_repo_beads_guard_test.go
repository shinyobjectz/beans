...
// Guardrail: ensure the cmd/bd test suite does not touch the real repo .beans state.
...
func TestMain(m *testing.M) {
	...
	// .beans files (like issues.jsonl via auto-sync) during test execution.
	...
	// .beans directory via git repo detection when there's a redirect file.
	// Each test that needs a .beans directory should set BEADS_DIR explicitly.
	...
	repoBeadsDir := filepath.Join(repoRoot, ".beans")
	...
	if diff := diffSnapshots(before, after); diff != "" {
		fmt.Fprintf(os.Stderr, "ERROR: test suite modified repo .beans state:\n%s\n", diff)
		if code == 0 {
			code = 1
		}
	}
...
// touches .beans files during test execution. Uses exec to avoid import cycles.
