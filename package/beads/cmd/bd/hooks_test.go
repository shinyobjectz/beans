...
func TestInstallHooksShared(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		sharedHooksDir := ".beans-hooks"
		for hookName := range hooks {
			hookPath := filepath.Join(sharedHooksDir, hookName)
			if _, err := os.Stat(hookPath); os.IsNotExist(err) {
				t.Errorf("Hook %s was not installed to .beans-hooks/", hookName)
			}
			if runtime.GOOS == "windows" {
				continue
			}
...
func TestHasBeadsJSONL(t *testing.T) {
	...
	// Should be false initially (no .beans directory)
	if hasBeadsJSONL() {
		t.Error("hasBeadsJSONL() = true, want false (no .beans)")
	}

	// Create .beans directory without any JSONL files
	if err := os.MkdirAll(".beans", 0755); err != nil {
	...
	// Create issues.jsonl
	if err := os.WriteFile(".beans/issues.jsonl", []byte("{}"), 0644); err != nil {
