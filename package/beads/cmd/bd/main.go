...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&dbPath, "db", "", "Database path (default: auto-discover .beans/*.db)")
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if dbPath == "" {
			...
			if foundDB := beads.FindDatabasePath(); foundDB != "" {
				...
				if cmd.Name() != "import" && cmd.Name() != "setup" && !isYamlOnlyConfigOp && !canAutoBootstrap {
					...
					if beadsDir != "" {
						...
						if _, err := os.Stat(jsonlPath); err == nil {
							...
							fmt.Fprintf(os.Stderr, "  • Add 'no-db: true' to .beans/config.yaml for permanent JSONL-only mode\n")
							os.Exit(1)
						}
					}
					...
					fmt.Fprintf(os.Stderr, "      or set BEADS_DIR to point to your .beans directory\n")
					os.Exit(1)
				}
				...
				//
				// IMPORTANT: Use FindBeadsDir() to get the correct .beans directory,
				// which follows redirect files. Without this, a redirected .beans
				// would create a local database instead of using the redirect target.
				// (GH#bd-0qel)
				targetBeadsDir := beads.FindBeadsDir()
				if targetBeadsDir == "" {
					targetBeadsDir = ".beans"
				}
		...
		if noDaemon {
			...
			// because all worktrees share the same .beans directory and the daemon
		...
		// dbPath is .beans/something.db, so workspace root is parent of .beans
		if dbPath != "" {
			beadsDir := filepath.Dir(dbPath)
