...
func cleanupMergeArtifacts(outputPath string, debug bool) {
	// Determine the .beans directory from the output path
	// outputPath is typically .beans/issues.jsonl
	...
	// 2. Run git clean -f in .beans/ directory to remove untracked files
	// (only if RepoContext available)
	if rcErr == nil {
		cleanCmd := rc.GitCmd(ctx, "clean", "-f")
		cleanCmd.Dir = beadsDir // Override to target .beans/ subdirectory specifically
		if debug {
