...
func (sm *SnapshotManager) readMetadata(path string) (*snapshotMetadata, error) {
	// #nosec G304 -- metadata lives under .beans and path is derived internally
...
func (sm *SnapshotManager) buildIDToLineMap(path string) (map[string]string, error) {
	...
	// #nosec G304 -- snapshot file lives in .beans/snapshots and path is derived internally
	f, err := os.Open(path)
	if err != nil {
		if os.IsNotExist(err) {
			return result, nil // Empty map for missing files
...
func (sm *SnapshotManager) copyFile(src, dst string) error {
	// #nosec G304 -- snapshot copy only touches files inside .beans/snapshots
	...
	// #nosec G304 -- snapshot copy only writes files inside .beans/snapshots
