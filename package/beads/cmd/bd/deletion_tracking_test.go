...
func TestMultiRepoDeletionTracking(t *testing.T) {
	...
	// Setup .beans directories
	primaryBeadsDir := filepath.Join(primaryDir, ".beans")
	additionalBeadsDir := filepath.Join(additionalDir, ".beans")
	...
	if err := os.MkdirAll(additionalBeadsDir, 0755); err != nil {
		t.Fatalf("Failed to create additional .beans dir: %v", err)
	}
...
func TestGetMultiRepoJSONLPaths_Duplicates(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(primaryDir, ".beans"), 0755); err != nil {
	...
	expectedJSONL := filepath.Join(primaryDir, ".beans", "issues.jsonl")
	for i, p := range paths {
		if p != expectedJSONL {
...
func TestGetMultiRepoJSONLPaths_PathsWithSpaces(t *testing.T) {
	...
	// Create .beans directories
	if err := os.MkdirAll(filepath.Join(primaryDir, ".beans"), 0755); err != nil {
		t.Fatalf("Failed to create primary .beans: %v", err)
	}
	if err := os.MkdirAll(filepath.Join(additionalDir, ".beans"), 0755); err != nil {
		t.Fatalf("Failed to create additional .beans: %v", err)
	}
	...
	expectedPrimary := filepath.Join(primaryDir, ".beans", "issues.jsonl")
	expectedAdditional := filepath.Join(additionalDir, ".beans", "issues.jsonl")
...
func TestGetMultiRepoJSONLPaths_RelativePaths(t *testing.T) {
	...
	expectedPrimary := filepath.Join(".", ".beans", "issues.jsonl")
	expectedOther := filepath.Join("../other", ".beans", "issues.jsonl")
	expectedBar := filepath.Join("./foo/../bar", ".beans", "issues.jsonl")
...
func TestGetMultiRepoJSONLPaths_TildeExpansion(t *testing.T) {
	...
	expectedPrimary := filepath.Join("~/repos/main", ".beans", "issues.jsonl")
	expectedAdditional := filepath.Join("~/repos/other", ".beans", "issues.jsonl")
...
func TestMultiRepoSnapshotIsolation(t *testing.T) {
	...
	// Create .beans/issues.jsonl in each
	repo1JSONL := filepath.Join(repo1Dir, ".beans", "issues.jsonl")
	repo2JSONL := filepath.Join(repo2Dir, ".beans", "issues.jsonl")

	if err := os.MkdirAll(filepath.Dir(repo1JSONL), 0755); err != nil {
		t.Fatalf("Failed to create repo1 .beans: %v", err)
	}
	if err := os.MkdirAll(filepath.Dir(repo2JSONL), 0755); err != nil {
		t.Fatalf("Failed to create repo2 .beans: %v", err)
	}
