...
func singleProcessOnlyBackend() bool {
	// Prefer dbPath if set; it points to either .beans/<db>.db (sqlite) or .beans/dolt (dolt dir).
	beadsDir := ""
	if dbPath != "" {
...
func shouldAutoStartDaemon() bool {
	...
	// .beans directory and the daemon would commit to the wrong branch.
...
// Note: socketPath parameter is unused - PID file is always in .beans directory
...
func getSocketPath() string {
	...
	// Get workspace path (parent of .beans directory)
	beadsDir := filepath.Dir(dbPath)
