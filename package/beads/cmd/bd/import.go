...
NOTE: Import requires direct database access and does not work with daemon mode.
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) > 0 {
			...
			fmt.Fprintf(os.Stderr, "  bd import -i .beans/issues.jsonl\n\n")
		...
		if input == "" && term.IsTerminal(int(os.Stdin.Fd())) {
			...
			fmt.Fprintf(os.Stderr, "  bd import -i .beans/issues.jsonl          # Import from file\n")
			fmt.Fprintf(os.Stderr, "  bd import -i .beans/issues.jsonl --dry-run # Preview changes\n")
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(trimmed, []byte("<<<<<<< ")) ||
				...
				if err := attemptAutoMerge(input); err != nil {
					...
					fmt.Fprintf(os.Stderr, "  git checkout --ours .beans/issues.jsonl && bd import -i .beans/issues.jsonl\n")
					fmt.Fprintf(os.Stderr, "  git checkout --theirs .beans/issues.jsonl && bd import -i .beans/issues.jsonl\n\n")
		...
		if err2 != nil || strings.TrimSpace(configuredPrefix) == "" {
			...
			if detectedPrefix == "" {
				...
				// But avoid using ".beans" as prefix - go up one level
				cwd, err := os.Getwd()
				if err != nil {
				...
				if dirName == ".beans" || dirName == "beads" {
					// Running from inside .beans/ - use parent directory
		...
		if result != nil && input != "" && (input == ".beans/issues.jsonl" || input == ".beans/beads.jsonl") {
