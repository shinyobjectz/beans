...
func TestSyncBranchPull_Success(t *testing.T) {
	...
	clone1BeadsDir := filepath.Join(clone1Dir, ".beans")
	...
	clone2BeadsDir := filepath.Join(clone2Dir, ".beans")
...
func TestSyncBranchMultipleConcurrentClones(t *testing.T) {
	...
	setupClone := func(name string) (string, *sqlite.SQLiteStorage) {
		...
		beadsDir := filepath.Join(cloneDir, ".beans")
	...
	jsonlPath1 := filepath.Join(clone1Dir, ".beans", "issues.jsonl")
	...
	jsonlPath2 := filepath.Join(clone2Dir, ".beans", "issues.jsonl")
	...
	jsonlPath3 := filepath.Join(clone3Dir, ".beans", "issues.jsonl")
...
// 1. The worktree's .beans directory triggers hook execution
...
func TestSyncBranchCommitAndPush_WithPreCommitHook(t *testing.T) {
	...
	// 1. Checks if .beans directory exists (like bd hook does)
...
func TestGitPushFromWorktree_FetchRebaseRetry(t *testing.T) {
	...
	beadsDir1 := filepath.Join(clone1Dir, ".beans")
	...
	runGitCmd(t, clone1Dir, "add", ".beans/issues.jsonl")
	...
	beadsDir2 := filepath.Join(worktreePath, ".beans")
	...
	runGitCmd(t, worktreePath, "add", ".beans/metadata.json")
...
func TestDaemonSyncBranchE2E(t *testing.T) {
	...
	withBeadsDir(t, machineA, func() {
		beadsDirA := filepath.Join(machineA, ".beans")
	...
	withBeadsDir(t, machineB, func() {
		beadsDirB := filepath.Join(machineB, ".beans")
