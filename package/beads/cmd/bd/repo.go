...
Configuration is stored in .beans/config.yaml under the 'repos' section:
...
The path should point to a directory containing a .beans folder.
Paths can be absolute or relative (they are stored as-is).

This modifies .beans/config.yaml, which is version-controlled and
shared across all clones of this repository.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Validate the repo path exists and has .beans
		beadsDir := filepath.Join(expandedPath, ".beans")
		if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
			return fmt.Errorf("no .beans directory found at %s - is this a beads repository?", expandedPath)
		}
...
var repoListCmd = &cobra.Command{
	...
	Long: `List all repositories configured in .beans/config.yaml.
