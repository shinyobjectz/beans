...
func init() {
	...
	repairCmd.Flags().StringVar(&repairPath, "path", ".", "Path to repository with .beans directory")
...
func runRepair(cmd *cobra.Command, args []string) {
	...
	beadsDir := filepath.Join(repairPath, ".beans")
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		if repairJSON {
			outputJSONAndExit(repairResult{
				Status: "error",
				Error:  fmt.Sprintf(".beans directory not found at %s", beadsDir),
			}, 1)
		}
		fmt.Fprintf(os.Stderr, "Error: .beans directory not found at %s\n", beadsDir)
		os.Exit(1)
	}
