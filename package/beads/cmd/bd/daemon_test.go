...
func TestGetPIDFilePath(t *testing.T) {
	...
	dbPath = filepath.Join(tmpDir, ".beans", "test.db")
	pidFile, err := getPIDFilePath()
	if err != nil {
		t.Fatalf("getPIDFilePath failed: %v", err)
	}

	expected := filepath.Join(tmpDir, ".beans", "daemon.pid")
	if pidFile != expected {
	...
	tests := []struct {
	...
	}{
		{
			...
			set: func(t *testing.T) (string, string, string) {
				...
				dbFile := filepath.Join(dbDir, ".beans", "test.db")
		...
		},
		{
			...
			set: func(t *testing.T) (string, string, string) {
				dbDir := t.TempDir()
				dbFile := filepath.Join(dbDir, ".beans", "test.db")
				return "", dbFile, filepath.Join(dbDir, ".beans", "daemon.log")
			},
		},
	}
...
func TestDaemonIntegration(t *testing.T) {
	...
	tmpDir := t.TempDir()
	dbDir := filepath.Join(tmpDir, ".beans")
