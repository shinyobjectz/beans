...
func TestSetupGitExclude_Worktree(t *testing.T) {
	...
	if !strings.Contains(string(content), ".beans/") {
		t.Errorf("main repo exclude missing .beans/ pattern: %s", content)
	}
	...
	if worktreeContent, err := os.ReadFile(worktreeExcludePath); err == nil {
		...
		if strings.Contains(string(worktreeContent), ".beans/") {
			t.Errorf("worktree exclude should not have .beans/ pattern (it was written to wrong location)")
		}
	}
...
func TestSetupGitExclude_RegularRepo(t *testing.T) {
	...
	if !strings.Contains(string(content), ".beans/") {
		t.Errorf("exclude file missing .beans/ pattern: %s", content)
	}
	if !strings.Contains(string(content), ".claude/settings.local.json") {
