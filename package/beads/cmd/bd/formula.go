...
Search paths (in order):
  1. .beans/formulas/ (project)
  2. ~/.beans/formulas/ (user)
  3. $GT_ROOT/.beans/formulas/ (orchestrator, if GT_ROOT set)
...
Search paths (in order of priority):
  1. .beans/formulas/ (project - highest priority)
  2. ~/.beans/formulas/ (user)
  3. $GT_ROOT/.beans/formulas/ (orchestrator, if GT_ROOT set)
...
func getFormulaSearchPaths() []string {
	...
	if cwd, err := os.Getwd(); err == nil {
		paths = append(paths, filepath.Join(cwd, ".beans", "formulas"))
	}

	// User-level formulas
	if home, err := os.UserHomeDir(); err == nil {
		paths = append(paths, filepath.Join(home, ".beans", "formulas"))
	}

	// Orchestrator formulas (via GT_ROOT)
	if gtRoot := os.Getenv("GT_ROOT"); gtRoot != "" {
		paths = append(paths, filepath.Join(gtRoot, ".beans", "formulas"))
	}
