...
var jsonlFilePaths = []string{
	".beans/issues.jsonl",
	".beans/deletions.jsonl",
	".beans/interactions.jsonl",
	".beans/beads.jsonl", // Legacy filename, kept for backwards compatibility
}
...
hook scripts installed in .beans/hooks/ (or .git/hooks/).
...
Configuration (.beans/config.yaml):
  hooks:
...
func saveExportState(beadsDir, worktreeRoot string, state *ExportState) error {
	...
	path := getExportStatePath(beadsDir, worktreeRoot)
	// #nosec G306 -- state file in .beans directory
...
// getHooksDir returns the hooks directory (.beans/hooks/ or .git/hooks/).
func getHooksDir() (string, error) {
	// First check for .beans/hooks/ (preferred location)
