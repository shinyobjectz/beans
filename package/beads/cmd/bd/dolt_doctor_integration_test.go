...
func TestDoltDoctor_NoSQLiteWarningsAfterInitAndCreate(t *testing.T) {
	...
	socketPath := filepath.Join(tmpDir, ".beans", "bd.sock")
	env := []string{
		"BEADS_TEST_MODE=1",
	...
	if _, err := os.Stat(filepath.Join(tmpDir, ".beans", "beads.db")); err == nil {
		t.Fatalf("unexpected sqlite database created in dolt mode: %s", filepath.Join(tmpDir, ".beans", "beads.db"))
	}
}
