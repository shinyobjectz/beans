...
func TestFindBeadsDir(t *testing.T) {
	...
	// Create temp directory with .beans and a valid project file
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
	...
	if found == "" {
		t.Error("Expected to find .beans directory")
	}
...
func TestFindBeadsDir_NotFound(t *testing.T) {
	// Create temp directory without .beans
	tmpDir := t.TempDir()

	t.Chdir(tmpDir)

	found := beads.FindBeadsDir()
	// findBeadsDir walks up to root, so it might find .beans in parent dirs
	// (e.g., user's home directory). Just verify it's not in tmpDir itself.
	if found != "" && filepath.Dir(found) == tmpDir {
		t.Errorf("Expected not to find .beans in tmpDir, but got %s", found)
	}
}
...
func TestFindBeadsDir_ParentDirectory(t *testing.T) {
	...
	// Create structure: tmpDir/.beans and tmpDir/subdir
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatalf("Failed to create .beans dir: %v", err)
	...
	found := beads.FindBeadsDir()
	if found == "" {
		t.Error("Expected to find .beans directory in parent")
	}
	// Use EvalSymlinks to handle /var vs /private/var on macOS
	expectedPath, _ := filepath.EvalSymlinks(beadsDir)
	foundPath, _ := filepath.EvalSymlinks(found)
...
func TestCheckGitForIssues_NoBeadsDir(t *testing.T) {
	// Use current directory which has git but change to somewhere without .beans
	tmpDir := t.TempDir()
	t.Chdir(tmpDir)

	count, path, _ := checkGitForIssues()
	if count != 0 || path != "" {
		t.Logf("No .beans dir: count=%d, path=%s (expected 0, empty)", count, path)
	}
}

// TestCheckGitForIssues_ParentHubNotInherited tests the GH#896 fix: when running
// bd init in a subdirectory of a hub, the parent hub's .beans should NOT be used
...
func TestCheckGitForIssues_ParentHubNotInherited(t *testing.T) {
	...
	// tmpDir/
	//   .beans/            <- parent hub (NOT a git repo)
	//     issues.jsonl
	//     config.yaml
	...
	// Create parent hub's .beans with issues
	parentBeadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(parentBeadsDir, 0755); err != nil {
		t.Fatalf("Failed to create parent .beans: %v", err)
	}
	...
	// because the parent .beans is outside the git root (newproject)
	count, path, _ := checkGitForIssues()
