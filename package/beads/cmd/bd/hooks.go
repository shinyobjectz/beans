...
Use --beads to install to .beans/hooks/ (recommended for Dolt backend).
Use --shared to install to a versioned directory (.beans-hooks/) that can be
  ...
  - prepare-commit-msg: Add agent identity trailers (for orchestrator agents)`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if jsonOutput {
			...
			if beadsHooks {
				fmt.Println("Hooks installed to: .beans/hooks/")
				fmt.Println("Git config set: core.hooksPath=.beans/hooks")
				fmt.Println()
			} else if shared {
				fmt.Println("Hooks installed to: .beans-hooks/")
				fmt.Println("Git config set: core.hooksPath=.beans-hooks")
				fmt.Println()
				fmt.Println("⚠️  Remember to commit .beans-hooks/ to share with your team!")
				fmt.Println()
			}
...
func installHooksWithOptions(embeddedHooks map[string]string, force bool, shared bool, chain bool, beadsHooks bool) error {
	var hooksDir string
	if beadsHooks {
		// Use .beans/hooks/ directory (preferred for Dolt backend)
		beadsDir := beads.FindBeadsDir()
		if beadsDir == "" {
			return fmt.Errorf("not in a beads workspace (no .beans directory found)")
		}
		hooksDir = filepath.Join(beadsDir, "hooks")
	} else if shared {
		// Use versioned directory for shared hooks
		hooksDir = ".beans-hooks"
	} else {
...
func configureSharedHooksPath() error {
	// Set git config core.hooksPath to .beans-hooks
	// Note: This may run before .beans exists, so it uses git.GetRepoRoot() directly
	repoRoot := git.GetRepoRoot()
	if repoRoot == "" {
		return fmt.Errorf("not in a git repository")
	}
	cmd := exec.Command("git", "config", "core.hooksPath", ".beans-hooks")
	cmd.Dir = repoRoot
...
func runPreCommitHook() int {
	...
	if _, err := os.Stat(".beans"); os.IsNotExist(err) {
	...
	if os.Getenv("BEADS_NO_AUTO_STAGE") != "" {
		...
		if len(unstaged) > 0 {
			...
			fmt.Fprintln(os.Stderr, "Run: git add .beans/")
			return 1
		}
	} else {
...
// hasBeadsJSONL checks if any JSONL file exists in .beans/.
...
func init() {
	...
	hooksInstallCmd.Flags().Bool("shared", false, "Install hooks to .beans-hooks/ (versioned) instead of .git/hooks/")
	hooksInstallCmd.Flags().Bool("chain", false, "Chain with existing hooks (run them before bd hooks)")
	hooksInstallCmd.Flags().Bool("beads", false, "Install hooks to .beans/hooks/ (recommended for Dolt backend)")
