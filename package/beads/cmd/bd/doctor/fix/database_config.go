...
// findActualJSONLFile scans .beans/ for the actual JSONL file in use.
...
func LegacyJSONLConfig(path string) error {
	...
	if cfg.JSONLExport == "beads.jsonl" && legacyExists && !canonicalExists {
		...
		if content, err := os.ReadFile(gitattrsPath); err == nil {
			if strings.Contains(string(content), ".beans/beads.jsonl") {
				newContent := strings.ReplaceAll(string(content), ".beans/beads.jsonl", ".beans/issues.jsonl")
...
// findActualDBFile scans .beans/ for the actual database file in use.
