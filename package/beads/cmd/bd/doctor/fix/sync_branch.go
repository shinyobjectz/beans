...
// SyncBranchGitignore sets git index flags to hide .beans/issues.jsonl from git status
...
func SyncBranchGitignore(path string) error {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		return fmt.Errorf(".beans directory not found at %s", beadsDir)
	}
	...
	for _, filename := range filesToHide {
		...
		changed, err := setGitIndexFlags(path, jsonlPath, ".beans/"+filename)
		if err != nil {
			return err
		}
		if changed {
	...
	if anyChanged {
		fmt.Println("  ✓ Set git index flags to hide .beans/*.jsonl from git status")
	}
	return nil
}
...
// ClearSyncBranchGitignore removes git index flags from .beans/*.jsonl files.
...
// HasSyncBranchGitignoreFlags checks if git index flags are set on .beans/issues.jsonl.
