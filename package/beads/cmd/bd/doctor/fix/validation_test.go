...
// that a .beans directory exists before attempting fixes.
// This replaces 10+ individual "missing .beans directory" subtests.
func TestFixFunctions_RequireBeadsDir(t *testing.T) {
	...
	for _, tc := range funcs {
		t.Run(tc.name, func(t *testing.T) {
			// Use a temp directory without .beans
			dir := t.TempDir()
			err := tc.fn(dir)
			if err == nil {
				t.Errorf("%s should return error for missing .beans directory", tc.name)
			}
		})
	}
}
