...
func TestUntrackedJSONL_E2E(t *testing.T) {
	t.Run("commits untracked JSONL files", func(t *testing.T) {
		...
		// Create untracked JSONL file in .beans
		jsonlPath := filepath.Join(dir, ".beans", "deletions.jsonl")
...
func TestMergeDriver_E2E(t *testing.T) {
	t.Run("sets correct merge driver config", func(t *testing.T) {
		...
		output := runGit(t, dir, "config", "--get", "merge.beans.driver")
	...
	t.Run("fixes incorrect config", func(t *testing.T) {
		...
		runGit(t, dir, "config", "merge.beans.driver", "bd merge %L %O %A %R")
...
func TestSyncBranchHealth_E2E(t *testing.T) {
	...
	t.Run("resets sync branch when ahead of main", func(t *testing.T) {
		...
		output := runGit(t, dir, "rev-list", "--count", "main..beans-sync")
		if !strings.Contains(output, "1") {
	...
	t.Run("resets sync branch when diverged from main", func(t *testing.T) {
		...
		aheadOutput := runGit(t, dir, "rev-list", "--count", "main..beans-sync")
...
func TestFixGitignore_PartialPatterns(t *testing.T) {
	...
	t.Run("validateBeadsWorkspace requires .beans directory", func(t *testing.T) {
		dir := t.TempDir()
...
// TestPermissionsWithWrongPermissions_E2E tests fixing wrong permissions on .beans
func TestPermissionsWithWrongPermissions_E2E(t *testing.T) {
	...
	t.Run("fixes .beans directory with wrong permissions", func(t *testing.T) {
		dir := t.TempDir()
		beadsDir := filepath.Join(dir, ".beans")
		if err := os.MkdirAll(beadsDir, 0755); err != nil {
			t.Fatal(err)
	...
	t.Run("handles .beans directory without write permission", func(t *testing.T) {
		dir := t.TempDir()
		beadsDir := filepath.Join(dir, ".beans")
		if err := os.MkdirAll(beadsDir, 0700); err != nil {
			t.Fatal(err)
		...
		// Create a test file in .beans
		testFile := filepath.Join(beadsDir, "test.txt")
		if err := os.WriteFile(testFile, []byte("test"), 0600); err != nil {
			t.Fatal(err)
		}

		// Make .beans read-only (no write, no execute)
