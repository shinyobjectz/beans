...
// Permissions fixes file permission issues in the .beans directory
func Permissions(path string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to stat .beans directory: %w", err)
	}

	// Skip permission fixes for symlinked .beans directories (common on NixOS with home-manager)
	if info.Mode()&os.ModeSymlink != 0 {
		return nil // Symlink permissions are not meaningful on Unix
	}

	// Ensure .beans directory has exactly 0700 permissions (owner rwx only)
	expectedDirMode := os.FileMode(0700)
	if info.Mode().Perm() != expectedDirMode {
		if err := os.Chmod(beadsDir, expectedDirMode); err != nil {
			return fmt.Errorf("failed to fix .beans directory permissions: %w", err)
		}
	}
