...
// UntrackedJSONL stages and commits untracked .beans/*.jsonl files.
...
func UntrackedJSONL(path string) error {
	...
	cmd := exec.Command("git", "status", "--porcelain", "--untracked-files=all", ".beans/")
	cmd.Dir = path
	...
	for _, file := range untrackedFiles {
		...
		// Verify file exists in .beans directory (security check)
