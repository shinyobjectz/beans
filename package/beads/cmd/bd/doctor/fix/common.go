...
func validateBeadsWorkspace(path string) error {
	...
	beadsDir := filepath.Join(absPath, ".beans")
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		return fmt.Errorf("not a beads workspace: .beans directory not found at %s", absPath)
	}
...
// resolveBeadsDir follows .beans/redirect files to find the actual beads directory.
