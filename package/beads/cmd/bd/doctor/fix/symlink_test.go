...
// when .beans directory is a symlink (common on NixOS with home-manager).
//
// Behavior being tested:
// - When .beans is a symlink, Permissions() should return nil without changing anything
...
func TestPermissions_SkipsSymlinkedBeadsDir(t *testing.T) {
	...
	// Create target .beans directory with wrong permissions
	targetDir := filepath.Join(tmpDir, "target-beads")
	if err := os.MkdirAll(targetDir, 0777); err != nil { // intentionally wrong permissions
		t.Fatal(err)
	}

	// Create workspace with symlinked .beans
	workspaceDir := filepath.Join(tmpDir, "workspace")
	if err := os.MkdirAll(workspaceDir, 0755); err != nil {
		t.Fatal(err)
	}

	symlinkPath := filepath.Join(workspaceDir, ".beans")
	...
	if err != nil {
		t.Fatalf("Permissions() returned error for symlinked .beans: %v", err)
	}
...
// symlinked database files, but .beans directory permissions are still fixed.
func TestPermissions_SkipsSymlinkedDatabase(t *testing.T) {
	skipOnWindows(t)
	tmpDir := t.TempDir()

	// Create real .beans directory with wrong permissions
	...
	// Verify .beans directory permissions WERE fixed (not a symlink)
	beadsInfo, err := os.Stat(beadsDir)
	if err != nil {
		t.Fatal(err)
	}
	if beadsInfo.Mode().Perm() != 0700 {
		t.Errorf("Expected .beans to have 0700 permissions, got %o", beadsInfo.Mode().Perm())
	}
...
func TestPermissions_FixesRegularFiles(t *testing.T) {
	...
	// Create .beans directory with wrong permissions
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0777); err != nil { // intentionally wrong
		t.Fatal(err)
	}
	...
	// Verify .beans directory now has 0700
	beadsInfo, err := os.Stat(beadsDir)
	if err != nil {
		t.Fatal(err)
	}
