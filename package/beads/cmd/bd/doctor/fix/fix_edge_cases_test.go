...
func TestValidateBeadsWorkspace_EdgeCases(t *testing.T) {
	t.Run("nested .beans directories", func(t *testing.T) {
		// Create a workspace with nested .beans directories
		dir := setupTestWorkspace(t)
		nestedDir := filepath.Join(dir, "subdir")
		nestedBeadsDir := filepath.Join(nestedDir, ".beans")
		if err := os.MkdirAll(nestedBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create nested .beans: %v", err)
		}
	...
	t.Run(".beans as a file not directory", func(t *testing.T) {
		dir := t.TempDir()
		beadsFile := filepath.Join(dir, ".beans")
		// Create .beans as a file instead of directory
		if err := os.WriteFile(beadsFile, []byte("not a directory"), 0600); err != nil {
			t.Fatalf("failed to create .beans file: %v", err)
		}

		err := validateBeadsWorkspace(dir)
		// NOTE: Current implementation only checks if .beans exists via os.Stat,
		...
		if err == nil {
			...
			t.Log(".beans exists as file - validation passes (edge case)")
		}
	})

	t.Run(".beans as symlink to directory", func(t *testing.T) {
		dir := t.TempDir()
		// Create actual .beans directory elsewhere
		...
		// Create symlink .beans -> actual_beads
		symlinkPath := filepath.Join(dir, ".beans")
		...
		if err := validateBeadsWorkspace(dir); err != nil {
			t.Errorf("expected symlinked .beans directory to be valid, got: %v", err)
		}
	})

	t.Run(".beans as symlink to file", func(t *testing.T) {
		dir := t.TempDir()
		// Create a file
		...
		if err == nil {
			t.Log(".beans symlink to file - validation passes (edge case)")
		}
	})

	t.Run(".beans as broken symlink", func(t *testing.T) {
		dir := t.TempDir()
		...
		err := validateBeadsWorkspace(dir)
		if err == nil {
			t.Error("expected error when .beans is a broken symlink")
		}
	})
...
func TestGitHooks_EdgeCases(t *testing.T) {
	...
	t.Run("git worktree with .git file", func(t *testing.T) {
		...
		// Create .beans in worktree
		beadsDir := filepath.Join(worktreeDir, ".beans")
		if err := os.MkdirAll(beadsDir, 0755); err != nil {
			t.Fatalf("failed to create .beans in worktree: %v", err)
		}
...
func TestMergeDriver_EdgeCases(t *testing.T) {
	...
	t.Run("succeeds after config was previously set", func(t *testing.T) {
		...
		cmd := exec.Command("git", "config", "merge.beans.driver")
		cmd.Dir = dir
...
func TestUntrackedJSONL_EdgeCases(t *testing.T) {
	t.Run("staged but uncommitted JSONL files", func(t *testing.T) {
		...
		jsonlFile := filepath.Join(dir, ".beans", "deletions.jsonl")
		if err := os.WriteFile(jsonlFile, []byte(`{"id":"test-1","ts":"2024-01-01T00:00:00Z","by":"user"}`+"\n"), 0600); err != nil {
			t.Fatalf("failed to create JSONL file: %v", err)
		}
		runGit(t, dir, "add", ".beans/deletions.jsonl")

		// Check git status - should show staged file
		output := runGit(t, dir, "status", "--porcelain", ".beans/")
		if !strings.Contains(output, "A  .beans/deletions.jsonl") {
	...
	t.Run("mixed tracked and untracked JSONL files", func(t *testing.T) {
		...
		trackedFile := filepath.Join(dir, ".beans", "issues.jsonl")
		if err := os.WriteFile(trackedFile, []byte(`{"id":"test-1"}`+"\n"), 0600); err != nil {
			t.Fatalf("failed to create tracked JSONL: %v", err)
		}
		runGit(t, dir, "add", ".beans/issues.jsonl")
		runGit(t, dir, "commit", "-m", "initial")

		// Create an untracked JSONL file
		untrackedFile := filepath.Join(dir, ".beans", "deletions.jsonl")
		...
		output = runGit(t, dir, "ls-files", ".beans/")
	...
	t.Run("JSONL file outside .beans directory is ignored", func(t *testing.T) {
		dir := setupTestGitRepo(t)
		...
		// Create a JSONL file outside .beans
		...
		if !strings.Contains(output, "?? data.jsonl") {
			t.Error("expected file outside .beans to remain untracked")
		}
	})
}
...
func TestMigrateTombstones_EdgeCases(t *testing.T) {
	t.Run("malformed deletions.jsonl with corrupt JSON", func(t *testing.T) {
		...
		deletionsPath := filepath.Join(dir, ".beans", "deletions.jsonl")
		jsonlPath := filepath.Join(dir, ".beans", "issues.jsonl")
...
func TestPermissions_EdgeCases(t *testing.T) {
	...
	t.Run("symbolic link to .beans directory", func(t *testing.T) {
		dir := t.TempDir()

		// Create actual .beans directory elsewhere
		actualBeadsDir := filepath.Join(t.TempDir(), "actual-beads")
		if err := os.MkdirAll(actualBeadsDir, 0755); err != nil {
			t.Fatalf("failed to create actual .beans: %v", err)
		}
		...
		err := Permissions(dir)
		if err != nil {
			t.Errorf("expected no error for symlinked .beans, got: %v", err)
		}

		// Verify target directory permissions were not changed
		info, err := os.Stat(actualBeadsDir)
		if err != nil {
			t.Fatalf("failed to stat actual .beans: %v", err)
		}
	...
	t.Run("symbolic link to database file", func(t *testing.T) {
		...
		// Create symlink to it
		dbSymlinkPath := filepath.Join(dir, ".beans", "beads.db")
	...
	t.Run("fixes incorrect .beans directory permissions", func(t *testing.T) {
		dir := setupTestWorkspace(t)
		...
		if err != nil {
			t.Fatalf("failed to stat .beans: %v", err)
		}
	...
	t.Run("fixes incorrect database file permissions", func(t *testing.T) {
		...
		dbPath := filepath.Join(dir, ".beans", "beads.db")
