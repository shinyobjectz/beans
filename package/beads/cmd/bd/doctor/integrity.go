...
// This detects when a .beans directory was copied from another repo or when
...
func CheckRepoFingerprint(path string) DoctorCheck {
	...
	if backend == configfile.BackendDolt {
		...
		if storedRepoID != currentRepoID {
			return DoctorCheck{
				...
				Fix:     "Run 'bd migrate --update-repo-id' if URL changed, or 'rm -rf .beans && bd init --backend dolt' if wrong database",
			}
		}
	...
	if storedRepoID != currentRepoID {
		return DoctorCheck{
			...
			Message: "Database belongs to different repository",
			Detail:  fmt.Sprintf("stored: %s, current: %s", storedRepoID[:8], currentRepoID[:8]),
			Fix:     "Run 'bd migrate --update-repo-id' if URL changed, or 'rm -rf .beans && bd init' if wrong database",
		}
	}
