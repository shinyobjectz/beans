...
func CheckMergeDriver(path string) DoctorCheck {
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Fix:     "Run 'bd init' to configure the merge driver, or manually: git config merge.beans.driver \"bd merge %A %O %A %B\"",
		}
	}
	...
	if strings.Contains(currentConfig, "%L") || strings.Contains(currentConfig, "%R") {
		return DoctorCheck{
			...
			Fix:     "Run 'bd doctor --fix' to update to correct config, or manually: git config merge.beans.driver \"bd merge %A %O %A %B\"",
		}
	}
	...
	if currentConfig != correctConfig {
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'bd doctor --fix' to update config, or manually: git config merge.beans.driver \"%s\"", correctConfig),
		}
	}
...
func CheckSyncBranchHealth(path string) DoctorCheck {
	...
	// The sync branch only tracks .beans/ data - source file differences are expected behavior.
