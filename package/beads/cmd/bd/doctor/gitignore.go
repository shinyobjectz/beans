...
// GitignoreTemplate is the canonical .beans/.gitignore content
...
# Worktree redirect file (contains relative path to main repo's .beans/)
...
// requiredPatterns are patterns that MUST be in .beans/.gitignore
...
// CheckGitignore checks if .beans/.gitignore is up to date
func CheckGitignore() DoctorCheck {
	gitignorePath := filepath.Join(".beans", ".gitignore")
	...
	if err != nil {
		return DoctorCheck{
			Name:    "Gitignore",
			Status:  "warning",
			Message: ".beans/.gitignore not found",
	...
	if len(missing) > 0 {
		return DoctorCheck{
			Name:    "Gitignore",
			Status:  "warning",
			Message: "Outdated .beans/.gitignore (missing merge artifact patterns)",
...
// FixGitignore updates .beans/.gitignore to the current template
...
func CheckIssuesTracking() DoctorCheck {
	issuesPath := filepath.Join(".beans", "issues.jsonl")
...
// CheckRedirectNotTracked verifies that .beans/redirect is NOT tracked by git.
...
func CheckRedirectNotTracked() DoctorCheck {
	redirectPath := filepath.Join(".beans", "redirect")
	...
	return DoctorCheck{
		...
		Detail:  "The .beans/redirect file contains a relative path that only works in this worktree. When committed, it causes warnings in other clones.",
		Fix:     "Run 'bd doctor --fix' to untrack, or manually: git rm --cached .beans/redirect",
	}
}

// FixRedirectTracking untracks the .beans/redirect file from git
...
func CheckRedirectTargetValid() DoctorCheck {
	...
	// Resolve the redirect path relative to the parent of .beans
	cwd, err := os.Getwd()
	if err != nil {
		return DoctorCheck{
			Name:    "Redirect Target Valid",
...
func CheckRedirectTargetSyncWorktree() DoctorCheck {
	...
	// The target is inside a .beans dir, so the repo is the parent of .beans
	targetRepoRoot := filepath.Dir(resolvedTarget)

	// Check for beads-sync worktree
	worktreePath := filepath.Join(targetRepoRoot, ".beans-sync")
...
// When a repo uses .beans/redirect, it doesn't need its own beads-sync worktree since
...
func CheckNoVestigialSyncWorktrees() DoctorCheck {
	...
	// Check for local .beans-sync worktree
	cwd, err := os.Getwd()
	if err != nil {
		return DoctorCheck{
			Name:    "Vestigial Sync Worktrees",
	...
	// Check for .beans-sync worktree
	syncWorktreePath := filepath.Join(gitRoot, ".beans-sync")
	...
	// Found a local .beans-sync but we have a redirect - this is vestigial
	return DoctorCheck{
		Name:    "Vestigial Sync Worktrees",
		Status:  StatusWarning,
		Message: "Vestigial .beans-sync worktree found",
...
// CheckLastTouchedNotTracked verifies that .beans/last-touched is NOT tracked by git.
...
func CheckLastTouchedNotTracked() DoctorCheck {
	lastTouchedPath := filepath.Join(".beans", "last-touched")
	...
	return DoctorCheck{
		...
		Detail:  "The .beans/last-touched file is local runtime state that should never be committed.",
		Fix:     "Run 'bd doctor --fix' to untrack, or manually: git rm --cached .beans/last-touched",
	}
}

// FixLastTouchedTracking untracks the .beans/last-touched file from git
...
// SetSyncBranchGitignoreFlags sets git index flags on .beans/*.jsonl files.
