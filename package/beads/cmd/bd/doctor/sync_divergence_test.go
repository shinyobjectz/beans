...
func TestCheckSyncDivergence(t *testing.T) {
	...
	t.Run("no beads directory", func(t *testing.T) {
		...
		// Don't use initRepo which creates .beans
	...
	t.Run("all synced", func(t *testing.T) {
		...
		commitFile(t, dir, ".beans/issues.jsonl", jsonlContent, "add issues")
	...
	t.Run("uncommitted beads changes", func(t *testing.T) {
		...
		// Create .beans with JSONL and commit it
		beadsDir := filepath.Join(dir, ".beans")
		if err := os.MkdirAll(beadsDir, 0755); err != nil {
			t.Fatal(err)
		}
