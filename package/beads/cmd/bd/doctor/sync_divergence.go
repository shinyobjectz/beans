...
// 3. Uncommitted .beans/ changes exist
func CheckSyncDivergence(path string) DoctorCheck {
	...
	// Check 3: Uncommitted .beans/ changes
...
func checkJSONLGitDivergence(path, beadsDir string) *SyncDivergenceIssue {
	...
	if err := cmd.Run(); err != nil {
		...
		return &SyncDivergenceIssue{
			...
			FixCommand:  "git add .beans/ && git commit -m 'sync beads'",
		}
	}
...
// checkUncommittedBeadsChanges checks if there are uncommitted changes in .beans/ directory.
func checkUncommittedBeadsChanges(path, beadsDir string) *SyncDivergenceIssue {
	...
	// Check for uncommitted changes in .beans/
	...
	if cfg, err := configfile.Load(beadsDir); err == nil && cfg != nil && cfg.GetBackend() == configfile.BackendDolt {
		fixCmd = "git add .beans/ && git commit -m 'sync beads'"
	}

	return &SyncDivergenceIssue{
		Type:        "uncommitted_beads",
		Description: fmt.Sprintf("Uncommitted .beans/ changes (%d file(s))", fileCount),
		FixCommand:  fixCmd,
	}
}
