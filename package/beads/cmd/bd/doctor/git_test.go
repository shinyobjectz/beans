...
func TestCheckMergeDriver(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "not a git repo",
			setup: func(t *testing.T, dir string) {
				// Just create .beans directory, no git
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				setupGitRepoInDir(t, dir)
				cmd := exec.Command("git", "config", "merge.beans.driver", "bd merge %L %O %A %R")
				cmd.Dir = dir
				if err := cmd.Run(); err != nil {
					t.Fatalf("failed to set git config: %v", err)
				}
...
// setupGitRepoInDir initializes a git repo in the given directory with .beans
...
func TestCheckMergeDriver_PartiallyConfigured(t *testing.T) {
	tests := []struct {
		...
		{
			name: "only merge.beans.name configured",
			setup: func(t *testing.T, dir string) {
				setupGitRepoInDir(t, dir)
				cmd := exec.Command("git", "config", "merge.beans.name", "Beads merge driver")
				cmd.Dir = dir
				if err := cmd.Run(); err != nil {
					t.Fatalf("failed to set git config: %v", err)
				}
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				setupGitRepoInDir(t, dir)
				cmd := exec.Command("git", "config", "merge.beans.driver", "/usr/local/bin/bd merge %A %O %A %B")
				cmd.Dir = dir
				if err := cmd.Run(); err != nil {
					t.Fatalf("failed to set git config: %v", err)
				}
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				setupGitRepoInDir(t, dir)
				cmd := exec.Command("git", "config", "merge.beans.driver", "bd merge %A %O")
				cmd.Dir = dir
				if err := cmd.Run(); err != nil {
					t.Fatalf("failed to set git config: %v", err)
				}
...
func TestCheckOrphanedIssues_NoBeadsDir(t *testing.T) {
	...
	// Initialize git repo WITHOUT creating .beans
	...
	if !strings.Contains(check.Message, "no .beans directory") {
		t.Errorf("expected message about no .beans directory, got %q", check.Message)
	}
}
...
func TestCheckOrphanedIssues_NoDatabase(t *testing.T) {
	...
	// Create .beans directory but no database
...
func TestCheckOrphanedIssues_NoOpenIssues(t *testing.T) {
	...
	// Create .beans directory and database
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatal(err)
	}
