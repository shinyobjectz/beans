	...
	tests := []struct {
	...
	}{
		{
			...
			setupFunc:     func(t *testing.T, tmpDir string) {
				// Create .beans directory but no .gitignore
		...
		{
			name: "fails gracefully when .beans directory doesn't exist",
			setupFunc: func(t *testing.T, tmpDir string) {
				// Don't create .beans directory
			},
			expectedPerms: 0,
			expectError:   true,
		},
...
func TestFixGitignore_FileOwnership(t *testing.T) {
	...
	if !dirInfo.IsDir() {
		t.Error(".beans should be a directory")
	}
}
...
func TestFixGitignore_Symlink(t *testing.T) {
	...
	// Create symlink at .beans/.gitignore pointing to target
	gitignorePath := filepath.Join(".beans", ".gitignore")
...
func TestCheckGitignore_VariousStatuses(t *testing.T) {
	tests := []struct {
		...
		{
			name: "missing .beans directory",
			setupFunc: func(t *testing.T, tmpDir string) {
				// Don't create .beans directory
			},
			expectedStatus: StatusWarning,
			expectedFix:    "Run: bd init (safe to re-run) or bd doctor --fix",
			description:    "returns warning when .beans directory doesn't exist",
		},
		{
			name: "missing .gitignore file",
			setupFunc: func(t *testing.T, tmpDir string) {
...
func TestFixGitignore_SubdirectoryGitignore(t *testing.T) {
	// This test verifies that FixGitignore only operates on .beans/.gitignore
	...
	// Create .beans directory and gitignore
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}

	// Create .beans/.gitignore with old content
	beadsGitignorePath := filepath.Join(".beans", ".gitignore")
	...
	// Verify .beans/.gitignore was updated
	beadsContent, err := os.ReadFile(beadsGitignorePath)
	if err != nil {
		t.Fatalf("Failed to read .beans/.gitignore: %v", err)
	}
	if string(beadsContent) != GitignoreTemplate {
		t.Error(".beans/.gitignore should be updated to template")
	}
...
func TestCheckRedirectNotTracked_NoFile(t *testing.T) {
	...
	// Create .beans directory but no redirect file
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}
...
func TestCheckRedirectNotTracked_FileExistsNotTracked(t *testing.T) {
	...
	// Create .beans directory with redirect file
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}
	redirectPath := filepath.Join(beadsDir, "redirect")
	if err := os.WriteFile(redirectPath, []byte("../../../.beans"), 0600); err != nil {
		t.Fatal(err)
	}
...
// validates the presence of sync state patterns in .beans/.gitignore.
// GH#974
...
func TestCheckLastTouchedNotTracked_NoFile(t *testing.T) {
	...
	// Create .beans directory but no last-touched file
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}
...
func TestCheckLastTouchedNotTracked_FileExistsNotTracked(t *testing.T) {
	...
	// Create .beans directory with last-touched file
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	}
...
// TestGitignoreTemplate_ContainsLastTouched verifies that the .beans/.gitignore template
...
// the presence of the last-touched pattern in .beans/.gitignore.
