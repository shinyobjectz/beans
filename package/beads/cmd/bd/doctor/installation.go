...
// CheckInstallation verifies that .beans directory exists
func CheckInstallation(path string) DoctorCheck {
	beadsDir := filepath.Join(path, ".beans")
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		return DoctorCheck{
			Name:    "Installation",
			Status:  StatusError,
			Message: "No .beans/ directory found",
	...
	return DoctorCheck{
		Name:    "Installation",
		Status:  StatusOK,
		Message: ".beans/ directory found",
	}
}

// CheckMultipleDatabases checks for multiple database files in .beans directory
func CheckMultipleDatabases(path string) DoctorCheck {
	// Follow redirect to resolve actual beads directory (bd-tvus fix)
	beadsDir := resolveBeadsDir(filepath.Join(path, ".beans"))
...
// CheckPermissions verifies that .beans directory and database are readable/writable
func CheckPermissions(path string) DoctorCheck {
	// Follow redirect to resolve actual beads directory (bd-tvus fix)
	beadsDir := resolveBeadsDir(filepath.Join(path, ".beans"))

	// Check if .beans/ is writable
	testFile := filepath.Join(beadsDir, ".doctor-test-write")
	if err := os.WriteFile(testFile, []byte("test"), 0600); err != nil {
		return DoctorCheck{
			Name:    "Permissions",
			Status:  StatusError,
			Message: ".beans/ directory is not writable",
...
// CheckUntrackedBeadsFiles checks for untracked .beans/*.jsonl files that should be committed.
...
func CheckUntrackedBeadsFiles(path string) DoctorCheck {
	...
	// Skip if .beans doesn't exist
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		return DoctorCheck{
			Name:    "Untracked Files",
			Status:  StatusOK,
			Message: "N/A (no .beans directory)",
		}
	}
	...
	// Run git status --porcelain to find untracked files in .beans/
	cmd := exec.Command("git", "status", "--porcelain", ".beans/")
	cmd.Dir = path
	...
	if len(untrackedJSONL) == 0 {
		return DoctorCheck{
			Name:    "Untracked Files",
			Status:  StatusOK,
			Message: "All .beans/*.jsonl files are tracked",
		}
	}
	...
	return DoctorCheck{
		...
		Fix:     "Run 'bd doctor --fix' to stage and commit untracked files, or manually: git add .beans/*.jsonl && git commit",
	}
}

// FixPermissions fixes file permission issues in the .beans directory
func FixPermissions(path string) error {
	return fix.Permissions(path)
}

// FixUntrackedJSONL stages and commits untracked .beans/*.jsonl files
