...
// This handles the redirect mechanism where .beans/redirect points to
...
// CheckStaleMQFiles detects legacy .beans/mq/*.json files from gastown.
...
func CheckStaleMQFiles(path string) DoctorCheck {
	...
	return DoctorCheck{
		Name:     "Legacy MQ Files",
		Status:   StatusWarning,
		Message:  fmt.Sprintf("%d stale .beans/mq/*.json file(s)", len(files)),
		Detail:   "Legacy gastown merge queue files (local only, safe to delete)",
		Fix:      "Run 'bd doctor --fix' to delete, or 'rm -rf .beans/mq/'",
		Category: CategoryMaintenance,
	}
}

// FixStaleMQFiles removes the legacy .beans/mq/ directory and all its contents.
