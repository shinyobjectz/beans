...
func CheckLegacyJSONLFilename(repoPath string, gastownMode bool) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Determine which file is current and remove the others:\n" +
			"  1. Check .beans/metadata.json for 'jsonl_export' setting\n" +
			"  2. Verify with 'git log .beans/*.jsonl' to see commit history\n" +
			"  3. Remove the unused file(s): git rm .beans/<unused>.jsonl\n" +
			"  4. Commit the change",
	}
}
		...
		if legacyExists && !canonicalExists {
			return DoctorCheck{
				...
				Fix: "Run 'bd doctor --fix' to auto-migrate, or manually:\n" +
					"  1. git mv .beans/beads.jsonl .beans/issues.jsonl\n" +
...
func CheckFreshClone(repoPath string) DoctorCheck {
	...
	case configfile.BackendDolt:
		// Dolt is directory-backed: treat .beans/dolt as the DB existence signal.
