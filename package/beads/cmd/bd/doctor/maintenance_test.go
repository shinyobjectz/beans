...
func TestCheckStaleMQFiles_NoMQDirectory(t *testing.T) {
	// Create temp directory with .beans but no mq subdirectory
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beans")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatalf("failed to create .beans dir: %v", err)
...
func TestCheckStaleMQFiles_EmptyMQDirectory(t *testing.T) {
	// Create temp directory with .beans/mq but no JSON files
	tmpDir := t.TempDir()
	mqDir := filepath.Join(tmpDir, ".beans", "mq")
	if err := os.MkdirAll(mqDir, 0755); err != nil {
		t.Fatalf("failed to create .beans/mq dir: %v", err)
	}
...
func TestCheckStaleMQFiles_WithJSONFiles(t *testing.T) {
	// Create temp directory with .beans/mq containing JSON files
	tmpDir := t.TempDir()
	mqDir := filepath.Join(tmpDir, ".beans", "mq")
	if err := os.MkdirAll(mqDir, 0755); err != nil {
		t.Fatalf("failed to create .beans/mq dir: %v", err)
	...
	if check.Message != "2 stale .beans/mq/*.json file(s)" {
