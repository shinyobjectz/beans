...
// we add .beans/issues.jsonl to .git/info/exclude so it won't be staged.
...
func ensureForkProtection() {
	...
	debug.Printf("Fork detected: .beans/issues.jsonl excluded from git staging")
}
...
func isAlreadyExcluded(excludePath string) bool {
	...
	return strings.Contains(string(content), ".beans/issues.jsonl")
}
