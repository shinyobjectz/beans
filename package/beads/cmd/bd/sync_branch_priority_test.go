...
// This is the regression test for GH#812: when .beans/ is gitignored on code branches
...
func TestSyncBranchBypassesGitHasBeadsChanges(t *testing.T) {
	...
	t.Run("CommitToSyncBranch detects changes even when main repo has gitignored beads", func(t *testing.T) {
		...
		// Recreate .beans directory on main branch (it may not exist after checkout)
		if err := os.MkdirAll(beadsDir, 0750); err != nil {
			t.Fatalf("Failed to recreate .beans dir: %v", err)
		}
