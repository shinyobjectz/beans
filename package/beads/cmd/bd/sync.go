...
// commitAndPushBeads commits and pushes .beans changes using the appropriate method.
...
The --full flag provides the legacy full sync behavior for backwards compatibility.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if jsonlPath == "" {
			FatalError("not in a bd workspace (no .beans directory found)")
		}
		...
		// Use inline import to avoid subprocess path resolution issues with .beans/redirect (bd-ysal)
		if importOnly {
			if dryRun {
		...
		if redirectInfo.IsRedirected {
			if sbc.IsConfigured() {
				...
				fmt.Println("   The owner of the target .beans directory handles sync-branch")
			} else {
...
// - Base state: Last successful sync (.beans/sync_base.jsonl)
...
func LoadSyncConflictState(beadsDir string) (*SyncConflictState, error) {
	...
	// #nosec G304 -- path is derived from the workspace .beans directory
