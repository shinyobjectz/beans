...
func setupDaemonLock(pidFile string, dbPath string, log daemonLogger) (*DaemonLock, error) {
	...
	// Detect nested .beans directories (e.g., .beans/.beans/.beans/)
	cleanPath := filepath.Clean(beadsDir)
	if strings.Contains(cleanPath, string(filepath.Separator)+".beans"+string(filepath.Separator)+".beans") {
		log.Error("nested .beans directory detected", "path", cleanPath)
		log.Info("hint: do not run 'bd daemon' from inside .beans/ directory")
		log.Info("hint: use absolute paths for BEADS_DB or run from workspace root")
		return nil, fmt.Errorf("nested .beans directory detected")
	}
