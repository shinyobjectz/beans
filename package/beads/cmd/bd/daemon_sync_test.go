...
func TestExportImportRoundTrip(t *testing.T) {
	...
	// Create new database
	dbPath2 := filepath.Join(tmpDir, ".beans", "beads2.db")
...
func TestUpdateExportMetadataMultiRepo(t *testing.T) {
	tmpDir := t.TempDir()
	dbPath := filepath.Join(tmpDir, ".beans", "beads.db")
	jsonlPath1 := filepath.Join(tmpDir, "repo1", ".beans", "issues.jsonl")
	jsonlPath2 := filepath.Join(tmpDir, "repo2", ".beans", "issues.jsonl")
...
func TestExportWithMultiRepoConfigUpdatesAllMetadata(t *testing.T) {
	...
	dbPath := filepath.Join(primaryDir, ".beans", "beads.db")
	primaryJSONL := filepath.Join(primaryDir, ".beans", "issues.jsonl")
	additionalJSONL := filepath.Join(additionalDir, ".beans", "issues.jsonl")
...
// 3. Main .beans/issues.jsonl correctly shows status:"tombstone"
