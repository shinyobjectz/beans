...
var createCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if repoPath != "." {
			...
			targetBeadsDirPath := filepath.Join(targetBeadsDir, ".beans")
			var err error
...
// findTownBeadsDir finds the town-level .beans directory (where routes.jsonl lives).
// It walks up from the current directory looking for a .beans directory with routes.jsonl.
func findTownBeadsDir() (string, error) {
	...
	for {
		...
		// Check if this .beans directory has routes.jsonl
	...
	return "", fmt.Errorf("no routes.jsonl found in any parent .beans directory")
}
...
// If the .beans directory doesn't exist, it creates it and initializes with
...
func ensureBeadsDirForPath(ctx context.Context, targetPath string, sourceStore storage.Storage) error {
	beadsDir := filepath.Join(targetPath, ".beans")
	...
	if err := os.MkdirAll(beadsDir, 0750); err != nil {
		return fmt.Errorf("cannot create .beans directory: %w", err)
	}
