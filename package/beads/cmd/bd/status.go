...
func getGitActivity(hours int) *RecentActivitySummary {
	...
	if rc, err := beads.GetRepoContext(); err == nil {
		cmd = rc.GitCmd(context.Background(), "log", "--since="+since, "--numstat", "--pretty=format:%H", ".beans/issues.jsonl")
	} else {
		cmd = exec.Command("git", "log", "--since="+since, "--numstat", "--pretty=format:%H", ".beans/issues.jsonl") // #nosec G204 -- bounded arguments for local git history inspection
	}
	...
	if rc, err := beads.GetRepoContext(); err == nil {
		cmd = rc.GitCmd(context.Background(), "log", "--since="+since, "-p", ".beans/issues.jsonl")
	} else {
		cmd = exec.Command("git", "log", "--since="+since, "-p", ".beans/issues.jsonl") // #nosec G204 -- bounded arguments for local git history inspection
	}
	output, err = cmd.Output()
	if err != nil {
		return nil
